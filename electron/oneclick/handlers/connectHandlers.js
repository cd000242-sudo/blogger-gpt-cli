...
function isBloggerLoginUrl(url) {
...
function normalizeClosedPageError(error) {
...
async function waitForOpenPage(page, ms) {
    if (isPageClosed(page))
        throw normalizeClosedPageError('Page closed');
    try {
...
async function extractBloggerBlogIdFromPage(page) {
...
async function getFirstBloggerBlogHref(page) {
    if (isPageClosed(page))
        throw normalizeClosedPageError('Page closed');
    return await page.evaluate(() => {
...
async function waitForBloggerLoginIfNeeded(state, page) {
    const startedAt = Date.now();
    while (!state.cancelled) {
        if (isPageClosed(page))
            throw normalizeClosedPageError('Page closed');
        const currentUrl = page.url();
        if (!isBloggerLoginUrl(currentUrl))
            return;
...
async function waitForBloggerBlogIdSelection(state, page) {
    const startedAt = Date.now();
...
function registerConnectHandlers() {
    ...
    electron_1.ipcMain.handle('oneclick:extract-blogger-blog-id', async () => {
        const platform = BLOGGER_ID_EXTRACT_KEY;
        if (instances_1.connectStateManager.isBusy(platform)) {
