...
exports.runCloudwaysInfraSetup = runCloudwaysInfraSetup;
 ...
 */
async function runCloudwaysInfraSetup(state, domain, email, waitForLogin, options) {
    ...
    try {
        ...
        const preferredAppId = (options?.preferredAppId || '').toString().trim();
        if (appIdMatch) {
            appId = appIdMatch[1];
            ...
            try {
                const appLinks = await page.locator(selectors_1.CLOUDWAYS_SELECTORS.appLink).all();
                const candidates = [];
                for (const link of appLinks) {
                    try {
                        const href = await link.getAttribute('href');
                        const m = href ? href.match(/\/apps\/(\d+)/) : null;
                        if (m && m[1] && !candidates.includes(m[1]))
                            candidates.push(m[1]);
                    }
                    catch { /* 무시 */ }
                }
                if (preferredAppId && candidates.includes(preferredAppId)) {
                    ...
                    state.candidateAppIds = candidates;
        ...
        try {
            ...
            if (domainAlreadyAdded) {
                ...
                // 도메인 입력 필드 찾기
                const domainInput = await page.locator(selectors_1.CLOUDWAYS_SELECTORS.domainInput).first();
                if (await domainInput.isVisible({ timeout: 5000 })) {
                    ...
                    // 추가 버튼 클릭
                    const addBtn = await page.locator(selectors_1.CLOUDWAYS_SELECTORS.addDomainBtn).first();
        ...
        try {
            ...
            await (0, browser_1.waitForPageStable)(page, 4000);
            // 이미 SSL이 설치되어 있는지 확인
            const sslInstalled = await page.locator(selectors_1.CLOUDWAYS_SELECTORS.certInstalledText).first();
            ...
            if (alreadyInstalled) {
                ...
                if (!emailFilled) {
                    try {
                        const emailInput = await page.locator(selectors_1.CLOUDWAYS_SELECTORS.emailInputFallback).first();
                ...
                if (!domainFilled) {
                    try {
                        const domainInput = await page.locator(selectors_1.CLOUDWAYS_SELECTORS.sslDomainInputFallback).first();
                ...
                if (emailFilled && domainFilled) {
                    ...
                    const installBtn = await page.locator(selectors_1.CLOUDWAYS_SELECTORS.installCertBtn).first();
                    if (await installBtn.isVisible({ timeout: 5000 })) {
                        await installBtn.click();
                        // 🟡 Issue #2 Fix: locator().or().waitFor() 사용 (waitForSelector는 text= 엔진 미지원)
                        try {
                            await page.locator(selectors_1.CLOUDWAYS_SELECTORS.certInstalledText)
                                .or(page.locator(selectors_1.CLOUDWAYS_SELECTORS.installedSuccessText))
                                .or(page.locator(selectors_1.CLOUDWAYS_SELECTORS.alertSuccess))
                                .or(page.locator(selectors_1.CLOUDWAYS_SELECTORS.successClass))
                                .first()
