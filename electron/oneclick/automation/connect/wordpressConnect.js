...
function normalizeWordPressSiteUrl(siteUrl) {
...
async function waitForWordPressAdmin(state, page, baseUrl) {
    const startedAt = Date.now();
...
async function waitForProfileReady(state, page, baseUrl) {
    const startedAt = Date.now();
    while (!state.cancelled && Date.now() - startedAt <= WORDPRESS_MANUAL_TIMEOUT_MS) {
...
async function waitForApplicationPasswordSection(state, page, results) {
    const startedAt = Date.now();
    while (!state.cancelled && Date.now() - startedAt <= WORDPRESS_MANUAL_TIMEOUT_MS) {
...
async function extractGeneratedApplicationPassword(page) {
    return await page.evaluate(() => {
        ...
        if (pwEl) {
            const val = pwEl.value || pwEl.textContent;
            return val?.trim() || '';
        }
...
async function waitForGeneratedApplicationPassword(state, page, results) {
    const startedAt = Date.now();
...
async function automateWordPressConnect(state, page, siteUrl) {
    ...
    if (needsLogin) {
        ...
        if (!loggedIn) {
            blockForManualAction(state, 1, 'WordPress 관리자 로그인을 확인하지 못했습니다. 열린 브라우저에서 로그인/2단계 인증/CAPTCHA를 완료한 뒤 다시 시도해주세요.', results);
            return;
        }
        state.stepStatus = 'running';
    ...
    if (!profileReady) {
        blockForManualAction(state, 3, 'WordPress 프로필 화면을 확인하지 못했습니다. 관리자 권한/보안 플러그인 확인을 완료한 뒤 다시 시도해주세요.', results);
        return;
    }
    ...
    if (!results['wordpressUsername']) {
        ...
        if (profileUsername)
            results['wordpressUsername'] = profileUsername;
    }
    if (!results['wordpressUsername']) {
        blockForManualAction(state, 3, 'WordPress 사용자명을 자동으로 확인하지 못했습니다. 프로필 화면의 사용자명 값을 앱 입력칸에 직접 저장해주세요.', results);
        return;
    }
    if (state.cancelled)
        return;
    ...
    try {
        ...
        if (!hasAppPasswordSection) {
            blockForManualAction(state, 4, 'Application Passwords 영역을 찾지 못했습니다. WordPress 5.6 이상, HTTPS, 관리자 권한, 보안 플러그인 차단 여부를 확인해주세요.', results);
            return;
        }
        // 앱 이름 입력
        const nameInput = await page.$(selectors_1.WORDPRESS_SELECTORS.newAppPasswordNameInput);
        if (!nameInput) {
            blockForManualAction(state, 4, 'Application Password 이름 입력칸을 찾지 못했습니다. 프로필 화면 하단에서 Application Passwords 영역이 열려 있는지 확인해주세요.', results);
            return;
        }
        if (!nameInput) {
        ...
        if (!addBtn) {
            blockForManualAction(state, 4, 'Application Password 추가 버튼을 찾지 못했습니다. 버튼이 보이면 직접 누르거나, 보안 플러그인/권한 설정을 확인해주세요.', results);
            return;
        }
        if (!addBtn) {
        ...
        if (!results['wordpressPassword']) {
            ...
            if (watchedPassword) {
                ...
                blockForManualAction(state, 4, 'Application Password가 생성됐지만 자동 추출하지 못했습니다. 화면에 보이는 비밀번호를 앱 입력칸에 직접 붙여넣고 저장해주세요.', results);
                return;
            }
        }
    }
