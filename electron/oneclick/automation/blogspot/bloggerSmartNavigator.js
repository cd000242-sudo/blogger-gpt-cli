...
exports.extractBloggerBlogId = extractBloggerBlogId;
exports.scanBloggerPage = scanBloggerPage;
exports.scanBestBloggerPage = scanBestBloggerPage;
exports.ensureBloggerLanding = ensureBloggerLanding;
exports.waitForBloggerBlogId = waitForBloggerBlogId;
exports.ensureBloggerTarget = ensureBloggerTarget;
const browser_1 = require("../../utils/browser");
const BLOGGER_HOME = 'https://www.blogger.com/';
const DEFAULT_WAIT_MS = 30000;
const POLL_MS = 1200;
function extractBloggerBlogId(url) {
    const patterns = [
...
function classifyByUrl(url, host) {
    if (/accounts\.google\.com|signin|ServiceLogin/i.test(url))
        return 'google-login';
    if (/(^|\.)support\.google\.com$|(^|\.)myaccount\.google\.com$/i.test(host))
        return 'google-detour';
    if (!/(^|\.)blogger\.com$/i.test(host))
        return 'unknown';
    if (/\/blog\/themes\/edit\/\d+/i.test(url))
        return 'blogger-theme-editor';
    if (/\/blog\/settings\/\d+/i.test(url))
        return 'blogger-settings';
    if (/\/blog\/posts\/\d+/i.test(url))
...
async function scanBloggerPage(page) {
    ...
    if (baseKind === 'blogger-other') {
        if (dom.looksCreate)
            kind = 'blogger-create';
        else if (dom.looksHome)
            kind = 'blogger-home';
    }
    const evidence = [...dom.evidence];
    if (blogId && !evidence.some(item => item.includes(blogId)))
        evidence.unshift(`Blog ID ${blogId}`);
    if (kind !== 'unknown')
        evidence.unshift(kind);
    return {
        page,
        kind,
        url,
...
async function scanBestBloggerPage(page) {
    const pages = (() => {
        try {
...
async function ensureBloggerLanding(state, page, reason = 'Blogger 화면을 확인합니다') {
    ...
    if (scan.kind === 'google-detour') {
        ...
        try {
            await scan.page.waitForLoadState?.('networkidle', { timeout: 8000 });
        }
        catch { /* Blogger can keep polling. */ }
...
async function waitForBloggerBlogId(state, page, options = {}) {
    ...
    while (Date.now() < deadline) {
        ...
        page = landing.page;
        if (landing.scan.blogId)
            return landing;
        state.message = 'Blogger 화면은 열렸지만 Blog ID를 아직 찾지 못했습니다. 블로그 목록이 보이면 사용할 블로그를 한 번 선택해 주세요.';
        await (0, browser_1.sleep)(POLL_MS);
    }
    return null;
}
function targetUrl(target, blogId) {
    if (target === 'settings')
        return `https://www.blogger.com/blog/settings/${blogId}`;
    if (target === 'theme-editor')
        return `https://www.blogger.com/blog/themes/edit/${blogId}`;
    if (target === 'posts')
        return `https://www.blogger.com/blog/posts/${blogId}`;
    return BLOGGER_HOME;
}
function matchesTarget(scan, target, blogId) {
    if (target === 'home')
        return scan.kind.startsWith('blogger-');
    if (blogId && scan.blogId && scan.blogId !== blogId)
        return false;
    if (target === 'settings')
        return scan.kind === 'blogger-settings';
    if (target === 'theme-editor')
        return scan.kind === 'blogger-theme-editor';
    if (target === 'posts')
        return scan.kind === 'blogger-posts';
    return false;
}
async function ensureBloggerTarget(state, page, target, blogId = '', options = {}) {
    let landing = await ensureBloggerLanding(state, page, options.reason || 'Blogger 목표 화면을 확인합니다');
    page = landing.page;
    let scan = landing.scan;
    if (matchesTarget(scan, target, blogId))
        return { ok: true, page, scan };
