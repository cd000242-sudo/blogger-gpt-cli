...
exports.setupMetaGaAds = setupMetaGaAds;
const browser_1 = require("../../../utils/browser");
const selectors_1 = require("../../../config/selectors");
/**
 * 메타태그 활성화, GA 측정 ID 설정, ads.txt 코드 설정을 수행한다.
 */
async function setupMetaGaAds(state, page, blogId, config) {
    state.currentStep = 3;
    ...
    try {
        ...
        try {
            const metaSection = await page.locator(selectors_1.BLOGGER_SELECTORS.metaTagSection).first();
            if (await metaSection.isVisible({ timeout: 3000 })) {
                await metaSection.click();
                await (0, browser_1.sleep)(1000);
                const metaToggle = await page.locator(selectors_1.BLOGGER_SELECTORS.metaToggle).first();
                ...
                if (config.blogDescription) {
                    const metaInput = await page.locator(selectors_1.BLOGGER_SELECTORS.metaTextareaOrInput).first();
        ...
        if (config.gaId) {
            ...
            try {
                const gaSection = await page.locator(selectors_1.BLOGGER_SELECTORS.gaSection).first();
                if (await gaSection.isVisible({ timeout: 3000 })) {
                    await gaSection.click();
                    await (0, browser_1.sleep)(1000);
                    const gaInput = await page.locator(selectors_1.BLOGGER_SELECTORS.gaInput).first();
        ...
        if (config.adsTxt) {
            ...
            try {
                const earningsLink = await page.locator(selectors_1.BLOGGER_SELECTORS.earningsLink).first();
                if (await earningsLink.isVisible({ timeout: 3000 })) {
                    await earningsLink.click();
                    await (0, browser_1.sleep)(2000);
                }
                const adsTxtSection = await page.locator(selectors_1.BLOGGER_SELECTORS.adsTxtSection).first();
                if (await adsTxtSection.isVisible({ timeout: 3000 })) {
                    await adsTxtSection.click();
                    await (0, browser_1.sleep)(1000);
                    const customToggle = await page.locator(selectors_1.BLOGGER_SELECTORS.adsTxtCustomToggle).first();
                    ...
                    const adsTxtInput = await page.locator(selectors_1.BLOGGER_SELECTORS.adsTxtTextarea).first();
