...
import {
  ensureBloggerLanding,
  ensureBloggerTarget,
  extractBloggerBlogId,
  scanBestBloggerPage,
  waitForBloggerBlogId,
} from '../bloggerSmartNavigator';

type ExistingBlogInspection = {
  blogId: string;
  currentUrl: string;
  pageTitle: string;
  blogTitle: string;
...
function extractBlogIdFromUrl(url: string): string {
  const smartId = extractBloggerBlogId(url);
  if (smartId) return smartId;
  const patterns = [
...
async function findExistingBlogOnPage(page: any): Promise<{ blogId: string; href: string; text: string } | null> {
  try {
...
async function ensureOnBloggerPage(state: SetupState, page: any): Promise<any> {
  try {
    const landing = await ensureBloggerLanding(state, page, 'Blogger 현재 화면을 판독합니다');
    if (landing.scan.kind.startsWith('blogger-') || landing.scan.kind === 'google-login') {
      return landing.page;
    }
    const currentUrl = page.url();
    const parsed = new URL(currentUrl);
    if (/(^|\.)blogger\.com/i.test(parsed.hostname)) return page;
    ...
    try { await page.waitForLoadState?.('networkidle', { timeout: 8000 }); } catch { /* 허용 */ }
    await sleep(2500);
    const recovered = await ensureBloggerLanding(state, page, 'Blogger 대시보드 로드 확인');
...
function summarizeInspection(inspection: ExistingBlogInspection): string {
  const parts = [
...
async function inspectExistingBlog(
  state: SetupState,
  page: any,
  blogId: string
): Promise<ExistingBlogInspection> {
  const inspection: ExistingBlogInspection = {
    blogId,
    currentUrl: page.url(),
    pageTitle: '',
    blogTitle: '',
  ...
  try {
    ...
    try { await page.waitForLoadState?.('networkidle', { timeout: 8000 }); } catch { /* Blogger는 계속 통신할 수 있어 timeout 허용 */ }
    await sleep(2500);

    const scan = await page.evaluate(() => {
      const text = (document.body?.innerText || '').replace(/\s+/g, ' ').trim();
      const titleInput = document.querySelector('input[aria-label*="제목"], input[aria-label*="Title"], input[placeholder*="제목"]') as HTMLInputElement | null;
      const publicLink = Array.from(document.querySelectorAll('a[href*=".blogspot.com"]') as NodeListOf<HTMLAnchorElement>)
        .map((a) => a.href)
        .find(Boolean) || '';
      const labels = [
...
export async function createBlog(
  ...
  try {
    ...
    await sleep(2000);
    const firstScan = await scanBestBloggerPage(page);
    page = firstScan.page;
    ...
    if (!blogId) {
      ...
      if (found?.blogId) blogId = found.blogId;
    }
    ...
    if (useExisting && !blogId) {
      ...
      try { await page.waitForLoadState?.('networkidle', { timeout: 8000 }); } catch { /* 허용 */ }
      await sleep(3500);
      const detected = await waitForBloggerBlogId(state, page, {
        timeoutMs: 45000,
    ...
    if (forceCreateNew) {
      const home = await ensureBloggerTarget(state, page, 'home', '', {
        timeoutMs: 30000,
    ...
    // blogId 재확인
    await sleep(2000);
    const afterCreateScan = await scanBestBloggerPage(page);
