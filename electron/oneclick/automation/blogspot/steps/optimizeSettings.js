...
async function optimizeSettings(state, page, blogId, config) {
    ...
    try {
        ...
        if (config.blogDescription) {
            ...
            try {
                const descSection = await page.locator(selectors_1.BLOGGER_SELECTORS.descriptionSection).first();
                if (await descSection.isVisible({ timeout: 3000 })) {
                    await descSection.click();
                    await (0, browser_1.sleep)(1000);
                    const descTextarea = await page.locator(selectors_1.BLOGGER_SELECTORS.descriptionTextarea).first();
        ...
        try {
            const commentSection = await page.locator(selectors_1.BLOGGER_SELECTORS.commentSection).first();
            if (await commentSection.isVisible({ timeout: 3000 })) {
                await commentSection.click();
                await (0, browser_1.sleep)(1000);
                const hideOption = await page.locator(selectors_1.BLOGGER_SELECTORS.hideOption).first();
        ...
        try {
            const timezoneSection = await page.locator(selectors_1.BLOGGER_SELECTORS.timezoneSection).first();
            if (await timezoneSection.isVisible({ timeout: 3000 })) {
                await timezoneSection.click();
                await (0, browser_1.sleep)(1000);
                const seoulOption = await page.locator(selectors_1.BLOGGER_SELECTORS.seoulOption).first();
