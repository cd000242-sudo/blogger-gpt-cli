 ...
 */
export async function insertMetaTagToBloggerTheme(
  page: any,
  metaTag: string,
  blogId: string,
  label: string
  ...
  try {
    ...
    if (blogId) {
      ...
      // HTML 편집 버튼
      const editHtmlBtn = await page.$(
        'button:has-text("HTML 편집"), button:has-text("Edit HTML")'
      );
      if (editHtmlBtn) {
    ...
    const inserted = await page.evaluate((tag: string) => {
      ...
      if (headMatch) {
        const insertPos =
          currentContent.indexOf(headMatch[0]) + headMatch[0].length;
        const newContent =
    ...
    if (inserted) {
      ...
      const saveBtn = await page.$(
        'button[aria-label*="저장"], button[aria-label*="Save"], button:has-text("💾")'
      );
      if (saveBtn) {
        try {
    ...
    console.log(
      `[ONECLICK] ⚠️ ${label} ���타태그 삽입 실패 — <head> 태그를 찾지 못함`
    );
    return false;
  } catch (e) {
