...
type PlaywrightBrowserCandidate = {
  label: string;
  options: Record<string, unknown>;
};

type LaunchBrowserConfig = BrowserConfig & {
...
export type BrowserReadinessResult = {
  ok: boolean;
  browser?: string;
  detail: string;
  fix?: string;
...
function requirePlaywright(): any {
  try {
...
function getLaunchCandidates(): PlaywrightBrowserCandidate[] {
  return [
...
async function installPlaywrightChromium(): Promise<InstallResult> {
...
function ensurePlaywrightChromiumInstalled(): Promise<InstallResult> {
...
function getBrowserFixMessage(): string {
 ...
 */
export async function checkBrowserReadiness(autoInstall = true): Promise<BrowserReadinessResult> {
  let pw: any;
  try {
