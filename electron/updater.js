 ...
 */
function scheduleRelaunchWatchdog() {
    ...
    try {
        ...
        const child = spawn('powershell.exe', ['-NoProfile', '-NonInteractive', '-WindowStyle', 'Hidden', '-Command', script], { detached: true, stdio: 'ignore', windowsHide: true });
        child.unref();
 ...
 */
function installDownloadedUpdateNow(reason) {
    const updater = getAutoUpdater();
    if (!updater || !downloadedVersion || installTriggered)
        return false;
    installTriggered = true;
    const attempt = (0, updater_attempt_1.recordAttempt)(attemptDir(), downloadedVersion);
 ...
 */
async function askThenInstall(version, detail) {
    try {
        const answer = await electron_1.dialog.showMessageBox({
            type: 'info',
            buttons: ['지금 재시작', '나중에'],
            defaultId: 0,
            cancelId: 1,
        ...
        if (answer.response !== 0) {
            ...
            electron_1.BrowserWindow.getAllWindows().forEach((w) => {
                if (!w.isDestroyed())
                    w.webContents.send('auto-update-event', { type: 'postponed', version });
            });
            return;
        }
...
function initAutoUpdaterEarly() {
    ...
    try {
        const prev = (0, updater_attempt_1.readAttempt)(attemptDir());
        if (prev && prev.version === electron_1.app.getVersion()) {
            (0, updater_attempt_1.clearAttempt)(attemptDir());
    ...
    updater.on('update-downloaded', (info) => {
        ...
        const prev = (0, updater_attempt_1.readAttempt)(attemptDir());
        if ((0, updater_attempt_1.isRepeatAttempt)(prev, version)) {
            console.log(`[Updater] v${version} 설치를 ${prev?.count}번 걸었는데 아직 그대로 — 조용히 되풀이하지 않고 묻는다`);
            void askThenInstall(version, '조금 전 자동 설치가 적용되지 않았습니다. Windows 권한 확인창에서 \'아니오\'를 눌렀거나 창이 닫혔을 수 있습니다.\n'
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                if (started)
                    console.log('[Updater] 8초가 지나도 종료되지 않음 — 사장님께 묻는다');
                void askThenInstall(version, '자동 설치가 되지 않아 직접 여쭙니다.\n[지금 재시작] 을 누르면 설치가 시작되고(권한 확인창이 뜨면 \'예\'), 끝나면 앱이 다시 열립니다.');
            }, 8000);
        }, 2000);
    });
...
function registerUpdaterHandlers() {
     ...
     */
    electron_1.ipcMain.handle('updater:restart-to-latest', async () => {
        const updater = getAutoUpdater();
        if (!updater)
            return { ok: false, error: '이 빌드에서는 자동 업데이트를 쓸 수 없습니다 (개발 모드).' };
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                ...
                const fail = (e) => { cleanup(); reject(e instanceof Error ? e : new Error(String(e?.message || e))); };
                ...
                catch (e) { /* 이미 받는 중이면 여기서 던진다 — 이벤트를 계속 기다린다 */ }
            });
            isUpdateInProgress = true;
