...
exports.checkLicenseWithAutoLogin = checkLicenseWithAutoLogin;
exports.setMainWindow = setMainWindow;
exports.startMetricsOAuthServer = startMetricsOAuthServer;
exports.handleMetricsCallback = handleMetricsCallback;
exports.setupAutoLoginHandlers = setupAutoLoginHandlers;
exports.exchangeOAuthToken = exchangeOAuthToken;
exports.startBloggerOAuthWithServer = startBloggerOAuthWithServer;
exports.handleBloggerCallback = handleBloggerCallback;
exports.startBloggerOAuthServer = startBloggerOAuthServer;
exports.stopBloggerOAuthServer = stopBloggerOAuthServer;
const electron_1 = require("electron");
const path = __importStar(require("path"));
const auto_login_manager_1 = require("../dist/utils/auto-login-manager");
let loginWindow = null;
let mainWindow = null;
// 🔥 Metrics OAuth (Search Console + GA4)
let metricsOAuthServer = null;
...
function createLoginWindow(parent) {
    return new Promise((resolve) => {
        ...
        console.log('[LOGIN] isPackaged:', electron_1.app.isPackaged);
      ...
      `;
        ...
        electron_1.ipcMain.removeHandler('login-success-signal');
        electron_1.ipcMain.handle('login-success-signal', loginSuccessHandler);
        // 로그인 성공 시 resolve 호출
        loginWindow.webContents.on('ipc-message', (_event, channel) => {
        ...
        electron_1.ipcMain.removeHandler('login-success-signal');
        electron_1.ipcMain.handle('login-success-signal', async () => {
 ...
 * 앱 시작 시 호출
 */
async function checkLicenseWithAutoLogin() {
    try {
        console.log('[AUTO-LOGIN] 자동 로그인 확인 시작...');
        // 1. 자동 로그인 시도
        const autoLoginResult = await (0, auto_login_manager_1.tryAutoLogin)();
 ...
 */
function startMetricsOAuthServer(onCodeReceived) {
    return new Promise((resolve) => {
        try {
            ...
            metricsOAuthServer = http.createServer((req, res) => {
            ...
            metricsOAuthServer.on('error', (err) => {
 ...
 */
async function handleMetricsCallback(code) {
    try {
        const fs = require('fs');
        ...
        const tokenPath = path.join(electron_1.app.getPath('userData'), 'metrics-token.json');
        ...
        try {
            if (fs.existsSync(tokenPath)) {
                ...
                if (existing && existing.refresh_token)
                    existingRefreshToken = String(existing.refresh_token);
            }
        }
        catch { }
        const tokenFileData = {
...
function setupAutoLoginHandlers() {
    ...
    electron_1.ipcMain.handle('load-auto-login-config', async () => {
        return (0, auto_login_manager_1.loadAutoLoginConfig)();
    });
}
/**
...
let bloggerOAuthServer = null;
const BLOGGER_OAUTH_PORT = 58392; // 고정 포트
/**
 * Blogger OAuth 시작 (로컬 서버 기반)
 */
async function startBloggerOAuthWithServer() {
    try {
        const fs = require('fs');
        const envPath = path.join(electron_1.app.getPath('userData'), '.env');
        if (!fs.existsSync(envPath)) {
...
async function handleBloggerCallback(code) {
    try {
        ...
        // 토큰 파일에 저장
        const tokenPath = path.join(electron_1.app.getPath('userData'), 'blogger-token.json');
        const tokenFileData = {
 ...
 */
function startBloggerOAuthServer(onCodeReceived) {
    return new Promise((resolve) => {
        try {
            const http = require('http');
            // 기존 서버 종료
            ...
            bloggerOAuthServer = http.createServer((req, res) => {
            ...
            bloggerOAuthServer.on('error', (err) => {
 ...
 * Blogger OAuth 서버 종료
 */
function stopBloggerOAuthServer() {
    if (bloggerOAuthServer) {
        try {
            bloggerOAuthServer.close();
        }
