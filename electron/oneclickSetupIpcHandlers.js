...
function getOrCreateState(platform, totalSteps) {
...
const connectStates = {};
function getOrCreateConnectState(platform) {
...
function resetConnectState(platform) {
    if (connectStates[platform]) {
        try {
...
let infraState = null;
const infraLoginResolvers = {};
function getOrCreateInfraState() {
    if (!infraState) {
        infraState = {
            currentStep: 0,
            totalSteps: 5,
...
function resetInfraState() {
    // 🔴 browser.close()는 runCloudwaysInfraSetup()의 finally 블록이 담당
    // 여기서는 state 레퍼런스만 해제
    infraState = null;
}
async function waitForInfraLogin(timeout = 300000) {
...
// ═══════════════════════════════════════════════
async function runCloudwaysInfraSetup(state, domain, email) {
...
async function waitForLogin(platform, timeout = 300000) {
...
// 플랫폼별 자동화 로직
// ═══════════════════════════════════════════════
async function runBlogspotSetup(state, adminUrl, blogspotConfig) {
    const { browser, page } = await launchBrowser();
    state.browser = browser;
    state.page = page;
...
async function runWordPressSetup(state, adminUrl) {
    const { browser, page } = await launchBrowser();
    state.browser = browser;
    state.page = page;
    try {
...
function registerOneclickSetupIpcHandlers() {
    ...
    const webmasterStates = {};
    function getOrCreateWebmasterState(engine, blogUrl) {
    ...
    function resetWebmasterState(engine) {
        if (webmasterStates[engine]) {
            try {
                webmasterStates[engine].browser?.close();
            }
            catch { /* ignore */ }
    ...
    // ═══════════════════════════════════════════════
    async function automateGoogleSearchConsole(state, page, blogUrl) {
        const results = {};
        // 1) GSC 열기
    ...
    async function automateNaverSearchAdvisor(state, page, blogUrl) {
        ...
        try {
            ...
            if (naverMetaTag && blogUrl.includes('blogspot.com')) {
                ...
                try {
                    const inserted = await insertMetaTagToBloggerTheme(bloggerPage, naverMetaTag, '', // blogId는 테마 편집에서 자동 탐색
                    '네이버');
    ...
    // ═══════════════════════════════════════════════
    async function automateDaumWebmaster(state, page, blogUrl) {
        const results = {};
    ...
    // 🟠 Bing 웹마스터도구 자동화
    // ═══════════════════════════════════════════════
    async function automateBingWebmaster(state, page, blogUrl) {
        const results = {};
        // 1) Bing 웹마스터도구 열기
        ...
        try {
            ...
            const bingMetaTag = await page.evaluate(() => {
                ...
                if (codeBlock) {
                    const text = codeBlock.textContent?.trim() || '';
                    if (text.includes('msvalidate.01'))
                        return text;
                }
                // input에서 추출
                const inputs = document.querySelectorAll('input[readonly], input[type="text"]');
                for (const input of inputs) {
                    const val = input.value;
                    if (val && val.includes('msvalidate.01'))
                        return val;
                }
                // 텍스트에서 정규식 추출
                const bodyText = document.body.innerText;
            ...
            if (bingMetaTag && blogUrl.includes('blogspot.com')) {
                ...
                const bloggerPage = await context.newPage();
                try {
                    const inserted = await insertMetaTagToBloggerTheme(bloggerPage, bingMetaTag, '', // blogId 자동 탐색
                    'Bing');
    ...
    electron_1.ipcMain.handle('oneclick:cancel-infra', async () => {
        if (infraState) {
            ...
            try {
                await infraState.browser?.close();
            }
            catch { /* ignore */ }
            resetInfraState();
        }
