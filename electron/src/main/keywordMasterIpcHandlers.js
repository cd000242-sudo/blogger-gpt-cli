...
exports.setupKeywordMasterHandlers = setupKeywordMasterHandlers;
// 키워드 마스터 IPC 핸들러
...
const naver_datalab_api_1 = require("../utils/naver-datalab-api");
const naver_searchad_api_1 = require("../utils/naver-searchad-api");
const naver_autocomplete_1 = require("../utils/naver-autocomplete");
const google_trends_api_1 = require("../utils/google-trends-api");
const youtube_data_api_1 = require("../utils/youtube-data-api");
const environment_manager_1 = require("../utils/environment-manager");
const timing_golden_finder_1 = require("../utils/timing-golden-finder");
const realtime_search_keywords_1 = require("../utils/realtime-search-keywords");
const rising_keyword_finder_1 = require("../utils/rising-keyword-finder");
const realtime_rising_detector_1 = require("../utils/realtime-rising-detector");
const nate_realtime_api_1 = require("../utils/nate-realtime-api");
const naver_search_crawler_1 = require("../utils/keyword-competition/naver-search-crawler");
const fresh_keywords_api_1 = require("../utils/mass-collection/fresh-keywords-api");
const keyword_trend_analyzer_1 = require("../utils/keyword-trend-analyzer");
const keyword_validator_1 = require("../utils/keyword-validator");
const licenseManager = __importStar(require("../utils/licenseManager"));
const generative_ai_1 = require("@google/generative-ai");
const category_longtail_keyword_hunter_1 = require("../utils/category-longtail-keyword-hunter");
const pro_traffic_keyword_hunter_1 = require("../utils/pro-traffic-keyword-hunter");
const lite_traffic_keyword_hunter_1 = require("../utils/lite-traffic-keyword-hunter");
const naver_news_crawler_1 = require("../utils/naver-news-crawler");
const api_reliability_1 = require("../utils/api-reliability");
const mdp_engine_1 = require("../utils/mdp-engine");
const related_keyword_cache_1 = require("../utils/related-keyword-cache");
const ultimate_niche_finder_1 = require("../utils/ultimate-niche-finder");
// 키워드 마스터 창 열기
let handlersSetup = false; // 중복 호출 방지 플래그
// 전역 중지 플래그 (키워드별로 관리)
const keywordDiscoveryAbortMap = new Map();
// 무제한 라이선스 체크 헬퍼 함수
function checkUnlimitedLicense() {
    ...
    try {
        ...
        const licensePath = path.join(electron_1.app.getPath('userData'), 'license', 'license.json');
...
function setupKeywordMasterHandlers() {
    ...
    handlerNames.forEach(name => {
        try {
            if (electron_1.ipcMain.listenerCount(name) > 0) {
    ...
    const existingHandlerCount = electron_1.ipcMain.listenerCount('open-keyword-master-window');
    if (existingHandlerCount > 0) {
        console.log(`[KEYWORD-MASTER] 기존 핸들러 ${existingHandlerCount}개 제거 중...`);
        electron_1.ipcMain.removeHandler('open-keyword-master-window');
    }
    electron_1.ipcMain.handle('open-keyword-master-window', async () => {
        try {
            ...
            const keywordWindow = new electron_1.BrowserWindow({
                width: 1200,
                height: 900,
                title: 'LEWORD - 키워드마스터',
                webPreferences: {
    ...
    // 중지 핸들러 추가
    electron_1.ipcMain.handle('stop-keyword-discovery', (_event, keyword) => {
        console.log(`[KEYWORD-MASTER] 중지 요청: "${keyword}"`);
        keywordDiscoveryAbortMap.set(keyword, true);
        return { success: true };
    });
    electron_1.ipcMain.handle('find-golden-keywords', async (event, keyword, options) => {
        // 라이선스 체크
        ...
        try {
            ...
            if (source === 'naver') {
                ...
                if (naverClientId && naverClientSecret) {
                    ...
                    const engine = new mdp_engine_1.MDPEngine({
                    ...
                    try {
                        ...
                        for await (const result of engine.discover(actualKeyword, discoveryOptions)) {
                            ...
                            allKeywords.push(formattedResult);
                            chunk.push(result);
                            totalAdded++;
                            // 50개마다 브라우저로 청크 전송
                            if (chunk.length >= 50) {
    ...
    electron_1.ipcMain.handle('get-trending-keywords', async (_event, source) => {
            ...
            if (source === 'naver') {
                ...
                try {
                        ...
                        if (newsResponse.ok) {
                            ...
                            (newsData.items || []).forEach((item) => {
                                ...
                                const words = cleanTitle
                                    ...
                                    .map((w) => w.replace(/(의|이|가|을|를|에|에서|로|으로|와|과|은|는|도|만)$/, ''))
                    ...
                    if (uniqueKeywords.length > 0) {
                        ...
                        try {
                            const rankingKeywords = await (0, naver_datalab_api_1.getNaverRankingKeywords)({
                                clientId: naverClientId,
                                clientSecret: naverClientSecret
                            });
                    ...
                    const keywordsWithData = await Promise.all(keywordsToProcess.map(async (item) => {
                        try {
                            ...
                            const totalVolume = (pcVolume !== null || mobileVolume !== null)
                            ...
                            const volumeToDocRatio = (typeof docCount === 'number' && docCount > 0 && typeof totalVolume === 'number' && totalVolume > 0)
                    ...
                    keywordsWithData.sort((a, b) => {
                        ...
                        if (bVol !== null && aVol === null)
                            return 1;
                        if (aVol !== null && bVol === null)
                            return -1;
                        if (aVol !== null && bVol !== null && bVol !== aVol)
                            return bVol - aVol;
                        return 0;
                    });
                ...
                try {
                    const envManager = environment_manager_1.EnvironmentManager.getInstance();
                    const env = envManager.getConfig();
                    const { getGoogleTrendKeywords } = await Promise.resolve().then(() => __importStar(require('../utils/google-trends-api')));
                ...
                try {
                    ...
                    const { getYouTubeTrendKeywords } = await Promise.resolve().then(() => __importStar(require('../utils/youtube-data-api')));
                    ...
                    const keywordsWithData = await Promise.all(youtubeTrends.slice(0, 20).map(async (item) => {
                        try {
                            ...
                            const volumeToDocRatio = (typeof docCount === 'number' && docCount > 0 && viewCount !== null && viewCount > 0)
                                ? (viewCount / docCount)
                                : null;
                            return {
                                keyword: item.keyword,
    ...
    // 키워드 순위 확인
    electron_1.ipcMain.handle('check-keyword-rank', async (_event, data) => {
    ...
    // 경쟁자 분석
    electron_1.ipcMain.handle('analyze-competitors', async (_event, keyword) => {
        ...
        try {
            ...
            const data = await response.json();
            const competitors = (data.items || []).map((item, index) => {
                // 제목에서 HTML 태그 제거
    ...
    electron_1.ipcMain.handle('add-schedule', async (_event, _schedule) => {
        // TODO: 데이터베이스에 스케줄 저장
        return { success: true, id: Date.now().toString() };
    });
    electron_1.ipcMain.handle('toggle-schedule', async (_event, _id, _enabled) => {
        // TODO: 데이터베이스에서 스케줄 활성화/비활성화
        return { success: true };
    });
    // 알림 관련, 대시보드 통계는 아래 구현됨
    // 실시간 검색어 통합 조회 - 중복 등록 방지
    if (!electron_1.ipcMain.listenerCount('get-realtime-keywords')) {
        electron_1.ipcMain.handle('get-realtime-keywords', async (_event, options) => {
            try {
                // 라이선스 체크
                const license = await licenseManager.loadLicense();
                if (!license || !license.isValid) {
                ...
                if (platform === 'all') {
                    ...
                    promises.push((async () => {
                        try {
                            const naverKeywords = await (0, realtime_search_keywords_1.getNaverRealtimeKeywords)(limit);
                    ...
                    promises.push((async () => {
                        try {
                            const { getZumRealtimeKeywordsWithPuppeteer } = await Promise.resolve().then(() => __importStar(require('../utils/zum-realtime-api')));
                    ...
                    promises.push((async () => {
                        try {
                            ...
                            const keywords = await (0, nate_realtime_api_1.getNateRealtimeKeywordsWithPuppeteer)(limit);
                            ...
                            // 폴백: axios 기반 크롤러
                            try {
                                const fallbackKeywords = await (0, realtime_search_keywords_1.getNateRealtimeKeywords)(limit);
                    ...
                    promises.push((async () => {
                        try {
                            const { getDaumRealtimeKeywordsWithPuppeteer } = await Promise.resolve().then(() => __importStar(require('../utils/daum-realtime-api')));
                    ...
                    results.forEach((res) => {
                        if (res.status === 'fulfilled') {
                            ...
                            if (p === 'naver')
                                result.naver = converted;
                            else if (p === 'zum')
                                result.zum = converted;
                            else if (p === 'nate')
                                result.nate = converted;
                            else if (p === 'daum')
                                result.daum = converted;
                    ...
                    if (platform === 'naver') {
                        try {
                            const naverKeywords = await (0, realtime_search_keywords_1.getNaverRealtimeKeywords)(limit);
                            result.naver = naverKeywords.map((kw) => ({
                                keyword: kw.keyword || kw.text || '',
                                rank: kw.rank || 0,
                        ...
                        try {
                            // Puppeteer 기반 크롤러 사용 (동적 콘텐츠 지원)
                            const nateKeywords = await (0, nate_realtime_api_1.getNateRealtimeKeywordsWithPuppeteer)(limit);
                            result.nate = nateKeywords.map((k, idx) => ({ ...k, rank: idx + 1 }));
                        }
                        catch (err) {
                            console.error(`[GET-REALTIME-KEYWORDS] ❌ Nate 수집 실패:`, err?.message || err);
                            // 폴백: axios 기반 크롤러
                            try {
                                result.nate = await (0, realtime_search_keywords_1.getNateRealtimeKeywords)(limit);
                            }
                            catch {
                                result.nate = [];
                            }
                        ...
                        try {
                            const { getDaumRealtimeKeywordsWithPuppeteer } = await Promise.resolve().then(() => __importStar(require('../utils/daum-realtime-api')));
                            const puppeteerKeywords = await getDaumRealtimeKeywordsWithPuppeteer(limit);
                            result.daum = puppeteerKeywords.map((kw) => ({
                                keyword: kw.keyword || kw.text || '',
                                rank: kw.rank || 0,
    ...
    if (!electron_1.ipcMain.listenerCount('get-google-trend-keywords')) {
    ...
    electron_1.ipcMain.handle('get-youtube-videos', async (_event, options) => {
        ...
        try {
            ...
            if (options?.categoryId === 'trending') {
                ...
                if (!searchResponse.ok) {
                    ...
                    const isQuotaError = searchResponse.status === 403 && (errorReason === 'quotaExceeded' ||
                ...
                if (!videosResponse.ok) {
                    ...
                    const errorMessage = errorData?.error?.message || '';
                    const errorJsonString = JSON.stringify(errorData);
                    const isQuotaError = videosResponse.status === 403 && (errorReason === 'quotaExceeded' ||
                        errorDomain === 'youtube.quota' ||
                        errorMessage.includes('exceeded your quota') ||
                        errorJsonString.includes('"reason":"quotaExceeded"') ||
                        errorJsonString.includes('"domain":"youtube.quota"'));
                ...
                let trendingVideos = videosData.items
                    ...
                    .filter((video) => video !== null && video.videoId && video.videoId.length > 0); // null 및 유효하지 않은 비디오 필터링
                // 시간당 조회수 기준으로 정렬 (급상승 영상)
                trendingVideos.sort((a, b) => b.viewsPerHour - a.viewsPerHour);
                // 순위 추가
                ...
                console.log('[KEYWORD-MASTER] 상위 3개 영상:', trendingVideos.slice(0, 3).map((v) => ({
                    title: v.title,
            ...
            if (!response.ok) {
                ...
                const errorJsonString = JSON.stringify(errorData);
                // Quota 초과 오류 처리 (정확한 감지)
                const isQuotaError = response.status === 403 && (errorReason === 'quotaExceeded' ||
                    errorDomain === 'youtube.quota' ||
                    errorMessage.includes('exceeded your quota') ||
            ...
            let allVideos = data.items.map((item, index) => {
                // 데이터 검증 및 로깅
                if (!item.id) {
            ...
            }).filter((video) => video.videoId && video.videoId.length > 0); // 유효한 비디오만 필터링
            ...
            console.log('[KEYWORD-MASTER] 최종 YouTube 영상 결과:', {
                ...
                sampleTitles: finalVideos.slice(0, 3).map((v) => v.title)
            });
            return {
                videos: finalVideos,
            ...
            const hasQuotaInMessage = errorMessage.includes('quotaExceeded') ||
                ...
                (errorMessage.includes('domain":"youtube.quota'));
            // JSON 문자열에서 확인
            const hasQuotaInJson = errorString.includes('quotaExceeded') ||
            ...
            const hasQuotaInErrorData = errorData?.error?.errors?.[0]?.reason === 'quotaExceeded' ||
                errorData?.error?.errors?.[0]?.domain === 'youtube.quota' ||
                (typeof errorData === 'object' && JSON.stringify(errorData).includes('quotaExceeded'));
            // statusCode 확인
            const hasQuotaStatusCode = error.statusCode === 403 && (errorMessage.includes('quota') ||
    ...
    electron_1.ipcMain.handle('get-keyword-schedules', async () => {
        try {
            ...
            // 키워드 관련 스케줄만 필터링
            const keywordSchedules = schedules.filter((s) => s.topic && s.keywords && s.keywords.length > 0);
            return keywordSchedules.map((s) => ({
                id: s.id,
    ...
    electron_1.ipcMain.handle('add-keyword-schedule', async (_event, scheduleData) => {
        try {
            const scheduleManager = require('../core/schedule-manager').getScheduleManager();
    ...
    electron_1.ipcMain.handle('toggle-keyword-schedule', async (_event, id, enabled) => {
        try {
            const scheduleManager = require('../core/schedule-manager').getScheduleManager();
    ...
    // 실시간 알림 관리
    const notificationsPath = path.join(electron_1.app.getPath('userData'), 'keyword-notifications.json');
    ...
    electron_1.ipcMain.handle('save-notification-settings', async (_event, settings) => {
        try {
    ...
    electron_1.ipcMain.handle('get-dashboard-stats', async () => {
        try {
            ...
            // 최근 키워드 분석 기록
            const keywordHistoryPath = path.join(electron_1.app.getPath('userData'), 'keyword-history.json');
            let keywordHistory = [];
            ...
            const recentTrendQueries = keywordHistory
                .filter((h) => h.type === 'trend')
                .slice(-10)
                .reverse();
            // 황금 키워드 발굴 이력
            const recentGoldenQueries = keywordHistory
                .filter((h) => h.type === 'golden')
                .slice(-10)
                .reverse();
            return {
                schedules: {
    ...
    // 키워드 그룹 관리
    const keywordGroupsPath = path.join(electron_1.app.getPath('userData'), 'keyword-groups.json');
    ...
    electron_1.ipcMain.handle('add-keyword-group', async (_event, group) => {
        try {
            let groups = [];
            if (fs.existsSync(keywordGroupsPath)) {
    ...
    electron_1.ipcMain.handle('update-keyword-group', async (_event, id, updates) => {
        try {
            let groups = [];
            if (fs.existsSync(keywordGroupsPath)) {
                groups = JSON.parse(fs.readFileSync(keywordGroupsPath, 'utf8'));
    ...
    electron_1.ipcMain.handle('delete-keyword-group', async (_event, id) => {
        try {
            let groups = [];
            if (fs.existsSync(keywordGroupsPath)) {
                groups = JSON.parse(fs.readFileSync(keywordGroupsPath, 'utf8'));
    ...
    if (!electron_1.ipcMain.listenerCount('get-sns-trends')) {
        electron_1.ipcMain.handle('get-sns-trends', async (_event, platform) => {
            // 무제한 라이선스 체크
            const licenseCheck = checkUnlimitedLicense();
            if (!licenseCheck.allowed) {
                return [];
            ...
            try {
                ...
                return await (async () => {
                    if (platform === 'instagram') {
                        ...
                        return await (0, google_trends_api_1.getGoogleTrendKeywords)();
                    }
    ...
    // - 3개월 이상 사용자: 무제한 사용
    // - 무료 사용자: 하루 5회 제한
    electron_1.ipcMain.handle('hunt-timing-gold', async (_event, options) => {
        // 파라미터 파싱 (객체 또는 문자열)
        ...
        const isPremium = license && license.isValid && (license.plan === '3months' ||
            license.plan === '1year' ||
        ...
        try {
            const result = await (0, lite_traffic_keyword_hunter_1.huntLiteTrafficKeywords)({
    ...
    if (false) {
        ...
        (async () => {
            const category = '';
            const finder = new timing_golden_finder_1.TimingGoldenFinder();
            const allKeywords = [];
            // === 구버전 코드 시작 ===
            try {
                const finder = new timing_golden_finder_1.TimingGoldenFinder();
                ...
                const refineKeyword = (rawKeyword) => {
                    if (!rawKeyword || rawKeyword.trim().length === 0)
                        return null;
                    ...
                    if (adPatterns.some(p => p.test(keyword)))
                        return null;
                    // 6. UI 텍스트 필터링
                    const uiWords = ['더보기', '전체보기', '검색', '로그인', '회원가입', '닫기', '홈', 'NOW'];
                    if (uiWords.includes(keyword))
                        return null;
                    return keyword;
                };
                // 1. 네이버 실시간 급상승 키워드 수집 (우선)
                try {
                    const naverRealtime = await (0, realtime_search_keywords_1.getAllRealtimeKeywords)();
                    ...
                    for (const item of naverKeywords.slice(0, 30)) { // 15개 -> 30개로 증가하여 더 풍부한 결과 제공
                        if (item.keyword && item.keyword.trim().length > 0) {
                            ...
                            const [blogResult, volumeResult] = await Promise.allSettled([
                                ...
                                Promise.race([
                                    (0, naver_datalab_api_1.getNaverKeywordSearchVolumeSeparate)({
                                        clientId: naverClientId,
                                        clientSecret: naverClientSecret
                                    }, [refinedKeyword]),
                ...
                // 3. Google 트렌드 키워드 수집
                try {
                    const googleTrends = await (0, google_trends_api_1.getGoogleTrendKeywords)();
                    for (const trend of googleTrends.slice(0, 30)) { // 15개 -> 30개로 증가
                        ...
                        if (naverClientId && naverClientSecret) {
                            ...
                            const [blogRes, volRes] = await Promise.allSettled([
                                ...
                                Promise.race([
                                    (0, naver_datalab_api_1.getNaverKeywordSearchVolumeSeparate)({ clientId: naverClientId, clientSecret: naverClientSecret }, [refinedKeyword]),
                                    new Promise((resolve) => setTimeout(() => resolve(null), 5000))
                                ])
                            ]);
                            if (blogRes.status === 'fulfilled') {
                                const v = blogRes.value;
                                googleDocCount = typeof v === 'number' ? v : null;
                            }
                            if (volRes.status === 'fulfilled' && volRes.value) {
                                const vd = volRes.value;
                                if (Array.isArray(vd) && vd.length > 0 && vd[0]) {
                                    const pc = typeof vd[0].pcSearchVolume === 'number' ? vd[0].pcSearchVolume : null;
                                    const mobile = typeof vd[0].mobileSearchVolume === 'number' ? vd[0].mobileSearchVolume : null;
                ...
                try {
                    ...
                    const youtubeApiKey = env.youtubeApiKey || process.env['YOUTUBE_API_KEY'] || '';
                    if (youtubeApiKey) {
                        const youtubeTrends = await (0, youtube_data_api_1.getYouTubeTrendKeywords)({ apiKey: youtubeApiKey });
                        for (const trend of youtubeTrends.slice(0, 20)) { // 10개 -> 20개로 증가
                            ...
                            let ytVolume = null;
                            let ytDocCount = null;
                            const envManager2 = environment_manager_1.EnvironmentManager.getInstance();
                            ...
                            if (naverClientId2 && naverClientSecret2) {
                                ...
                                const [blogRes2, volRes2] = await Promise.allSettled([
                                    ...
                                    Promise.race([
                                        (0, naver_datalab_api_1.getNaverKeywordSearchVolumeSeparate)({ clientId: naverClientId2, clientSecret: naverClientSecret2 }, [refinedKeyword]),
                                        new Promise((resolve) => setTimeout(() => resolve(null), 5000))
                                    ])
                                ]);
                                if (blogRes2.status === 'fulfilled') {
                                    const v = blogRes2.value;
                                    ytDocCount = typeof v === 'number' ? v : null;
                                }
                                if (volRes2.status === 'fulfilled' && volRes2.value) {
                                    const vd = volRes2.value;
                                    if (Array.isArray(vd) && vd.length > 0 && vd[0]) {
                                        const pc = typeof vd[0].pcSearchVolume === 'number' ? vd[0].pcSearchVolume : null;
                                        const mobile = typeof vd[0].mobileSearchVolume === 'number' ? vd[0].mobileSearchVolume : null;
                ...
                if (allKeywords.length === 0) {
                    ...
                    const realtimeData = await (0, realtime_search_keywords_1.getAllRealtimeKeywords)();
                    ...
                    const resultsWithRealData = await Promise.all(naverKeywords.slice(0, 15).map(async (item, idx) => {
                        ...
                        try {
                            ...
                            if (naverClientId && naverClientSecret) {
                                const blogResponse = await axios_1.default.get('https://openapi.naver.com/v1/search/blog.json', {
                                ...
                                const rawTotal = blogResponse?.data?.total;
                        ...
                        try {
                            ...
                            if (searchAdCustomerId && searchAdApiKey && searchAdSecretKey) {
                                ...
                                const hmac = (0, crypto_1.createHmac)('sha256', searchAdSecretKey);
                                hmac.update(`${timestamp}.${method}.${uri}`);
                                const signature = hmac.digest('base64');
                                const searchAdResponse = await axios_1.default.get(`https://api.searchad.naver.com${uri}`, {
                                    params: {
                                ...
                                const keywordData = searchAdResponse.data?.keywordList?.find((k) => k.relKeyword?.toLowerCase() === keyword.toLowerCase()) || searchAdResponse.data?.keywordList?.[0];
                                if (keywordData) {
                                    const parseCnt = (v) => {
                        ...
                        if (goldenRatio >= 50)
                            ...
                            score += 10;
                        if (searchVolumeForCalc >= 100000)
                            score += 10;
                        if (typeof documentCount === 'number' && documentCount < 50000)
                            score += 10;
                    ...
                    try {
                        ...
                        try {
                            trendAnalysis = await Promise.race([
                                (0, keyword_trend_analyzer_1.analyzeKeywordTrendingReason)(keyword.keyword, {
                        ...
                        // 키워드에 분석 결과 추가
                        keyword.trendingReason = trendAnalysis.trendingReason || '최근 검색 트렌드 급상승 중';
                        keyword.whyNow = trendAnalysis.whyNow || '검색량 급상승 중으로 조기 진입 효과 기대';
                        // 2. 연관 키워드, 연상 키워드 수집 및 검증
                        let relatedKeywords = [];
                        let associativeKeywords = [];
                        ...
                        if (naverClientId && naverClientSecret) {
                            try {
                                ...
                                while (retryCount < maxRetries && !success) {
                                    try {
                                        // 타임아웃 설정 (5초)
                                        const relatedPromise = (0, naver_datalab_api_1.getNaverRelatedKeywords)(keyword.keyword, {
                                            clientId: naverClientId,
                                            clientSecret: naverClientSecret
                                        }, { limit: 10 });
                                        const associativePromise = (0, naver_autocomplete_1.getNaverAutocompleteKeywords)(keyword.keyword, {
                                            clientId: naverClientId,
                                            clientSecret: naverClientSecret
                                        });
                                        ...
                                        const allRelatedKeywords = [
                                            ...(related || []).slice(0, 10).map((k) => k.keyword || k),
                                        ...
                                        const validatedKeywords = await Promise.race([
                                            (0, keyword_validator_1.validateKeywords)(keywordsToValidate, 2), // 재시도 횟수 줄임
                                            new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), 5000))
                                        ]);
                                        // 검증된 키워드 분류
                                        const validated = (validatedKeywords || []).filter((v) => v.validated);
                                        // 연관 키워드 (연관도가 높은 것들)
                                        relatedKeywords = validated.slice(0, 5).map((v) => ({
                                            keyword: v.keyword,
                                        ...
                                        // 연상 키워드 (연상 정도가 높은 것들)
                                        associativeKeywords = validated.slice(5, 10).map((v) => ({
                                            keyword: v.keyword,
                                            searchVolume: typeof v.searchVolume === 'number' ? v.searchVolume : null,
                                            documentCount: typeof v.documentCount === 'number' ? v.documentCount : null,
                                            validated: true
                                        ...
                                        const recommended = validated
                                            .filter((v) => (typeof v.searchVolume === 'number' && v.searchVolume > 100) && (typeof v.documentCount === 'number' && v.documentCount < 1000))
                                            .sort((a, b) => {
                                            const aVol = typeof a.searchVolume === 'number' ? a.searchVolume : null;
                        ...
                        // 4. 연관 키워드 정보 추가
                        timingScore.relatedKeywords = relatedKeywords;
                        timingScore.associativeKeywords = associativeKeywords;
                        timingScore.suggestedKeywords = suggestedKeywords;
                        return timingScore;
                    }
                    catch (error) {
                        ...
                        try {
                            ...
                            basicScore.trendingReason = `검색량 ${fallbackSearchVolume}회, 급상승률 ${Math.round(keyword.growthRate || 0)}%로 트래픽 폭발 가능성 높음`;
                            basicScore.whyNow = `경쟁 문서 ${fallbackDocCount}개로 적어 조기 진입 시 상위 노출 가능성 높음`;
                            basicScore.relatedKeywords = [];
                            basicScore.associativeKeywords = [];
                            basicScore.suggestedKeywords = [];
                            return basicScore;
                        }
                        catch (fallbackError) {
    ...
    electron_1.ipcMain.handle('infinite-keyword-search', async (event, options) => {
        try {
            ...
            const isKeywordCandidate = (value) => {
                ...
                if (value.includes('http') || value.includes('www'))
                    return false;
                if (value.includes('더보기') || value.includes('로그인') || value.includes('전체보기'))
                    return false;
            ...
            const processedForExtraction = new Set();
            let extractionDepth = 0;
            // 시작 키워드를 포함하여 연관 키워드 추출
            const extractRelatedKeywords = async (keyword, depth = 0, maxDepth = 3) => {
                // 무제한이 아닐 때만 개수 체크
                ...
                if (processedForExtraction.has(keyword))
                    return;
                    ...
                    if (response.ok) {
                        ...
                        items.forEach((item) => {
                            ...
                            if (description.includes(keyword)) {
                                const descWords = description.split(/[\s|,，、·\[\]()【】「」<>]+/).filter((w) => w.trim().length > 0);
                                const descKeywordIdx = descWords.findIndex((w) => w.includes(keyword));
            ...
            if (searchAdEnabled && searchAdConfig) {
                ...
                for (const seed of suggestionSeeds) {
                    if (!seed)
                        continue;
                    if (!isUnlimited && uniqueKeywords.length >= effectiveMaxKeywords)
                        break;
                    try {
                        const suggestions = await (0, naver_searchad_api_1.getNaverSearchAdKeywordSuggestions)(searchAdConfig, seed, Math.min(300, effectiveMaxKeywords * 2));
                        for (const suggestion of suggestions) {
                            const normalizedSuggestion = normalizeKeywordTerm(suggestion.keyword);
                            if (!normalizedSuggestion || !isKeywordCandidate(normalizedSuggestion))
                                continue;
                            if (uniqueKeywordSet.has(normalizedSuggestion.toLowerCase()))
                                continue;
            ...
            const processKeyword = async (keyword) => {
                try {
                    ...
                    const volumePromise = (0, naver_datalab_api_1.getNaverKeywordSearchVolumeSeparate)({
                        clientId: naverClientId,
                        clientSecret: naverClientSecret
                    }, [keyword]);
                    const relatedPromise = (0, related_keyword_cache_1.getRelatedKeywords)(keyword);
                    const volumeData = await volumePromise;
                    let pcVolume = null;
                    let mobileVolume = null;
                    ...
                    // 3. 비율 계산: 문서수 / 월간총검색량
                    const competitionRatio = (typeof totalVolume === 'number' && totalVolume > 0 && typeof documentCount === 'number')
            ...
            for (let i = 0; i < finalKeywords.length; i += batchSize) {
                ...
                const batchResults = await Promise.allSettled(batch.map(keyword => processKeyword(keyword)));
    ...
    // 엑셀 파일 저장 핸들러
    electron_1.ipcMain.handle('export-keywords-to-excel', async (event, data) => {
        try {
    ...
    // API 키 확인 핸들러
    if (!electron_1.ipcMain.listenerCount('check-api-keys')) {
    ...
    if (!electron_1.ipcMain.listenerCount('test-api-keys')) {
        electron_1.ipcMain.handle('test-api-keys', async () => {
            try {
                const envManager = environment_manager_1.EnvironmentManager.getInstance();
                const env = envManager.getConfig();
                const results = {
                ...
                if (env.naverClientId && env.naverClientSecret) {
                    ...
                    try {
                        const { getNaverAutocompleteKeywords } = await Promise.resolve().then(() => __importStar(require('../utils/naver-autocomplete')));
                ...
                if (env.naverSearchAdAccessLicense && env.naverSearchAdSecretKey && env.naverSearchAdCustomerId) {
                    ...
                    try {
                        const { getNaverSearchAdKeywordSuggestions } = await Promise.resolve().then(() => __importStar(require('../utils/naver-searchad-api')));
    ...
    if (!electron_1.ipcMain.listenerCount('api-health-check')) {
        electron_1.ipcMain.handle('api-health-check', async () => {
            try {
                console.log('[API-HEALTH] API 연결 상태 확인 시작...');
                const healthResult = await (0, api_reliability_1.apiHealthCheck)();
                const envManager = environment_manager_1.EnvironmentManager.getInstance();
                const env = envManager.getConfig();
                // API 키 구성 상태
                const apiStatus = {
    ...
    // 🔥 캐시 초기화 핸들러
    if (!electron_1.ipcMain.listenerCount('clear-api-cache')) {
        electron_1.ipcMain.handle('clear-api-cache', async (_event, pattern) => {
            try {
                (0, api_reliability_1.clearCache)(pattern);
    ...
    // 🔗 외부 URL 열기 핸들러
    if (!electron_1.ipcMain.listenerCount('open-external-url')) {
    ...
    if (!electron_1.ipcMain.listenerCount('clipboard-write-text')) {
        electron_1.ipcMain.handle('clipboard-write-text', async (_event, text) => {
            try {
    ...
    if (!electron_1.ipcMain.listenerCount('get-env')) {
    ...
    if (!electron_1.ipcMain.listenerCount('save-env')) {
        electron_1.ipcMain.handle('save-env', async (_event, env) => {
            try {
    ...
    if (!electron_1.ipcMain.listenerCount('is-developer-mode')) {
    ...
    if (!electron_1.ipcMain.listenerCount('is-packaged')) {
        electron_1.ipcMain.handle('is-packaged', async () => {
            console.log('[KEYWORD-MASTER] is-packaged:', electron_1.app.isPackaged);
            return electron_1.app.isPackaged;
        });
        console.log('[KEYWORD-MASTER] ✅ is-packaged 핸들러 등록 완료');
    }
    // 키워드 설정 저장 핸들러 (save-env와 동일하게 처리)
    if (!electron_1.ipcMain.listenerCount('save-keyword-settings')) {
        electron_1.ipcMain.handle('save-keyword-settings', async (event, settings) => {
            try {
                ...
                if (settings.naverClientId)
                    envConfig.naverClientId = settings.naverClientId;
                if (settings.naverClientSecret)
                    envConfig.naverClientSecret = settings.naverClientSecret;
                if (settings.youtubeApiKey)
                    envConfig.youtubeApiKey = settings.youtubeApiKey;
                if (settings.naverSearchAdAccessLicense)
                    envConfig.naverSearchAdAccessLicense = settings.naverSearchAdAccessLicense;
                if (settings.naverSearchAdSecretKey)
                    envConfig.naverSearchAdSecretKey = settings.naverSearchAdSecretKey;
                if (settings.naverSearchAdCustomerId)
                    envConfig.naverSearchAdCustomerId = settings.naverSearchAdCustomerId;
                // 설정 저장
    ...
    if (!electron_1.ipcMain.listenerCount('crawl-url')) {
        electron_1.ipcMain.handle('crawl-url', async (_event, url) => {
            try {
    ...
    if (!electron_1.ipcMain.listenerCount('transform-content')) {
        electron_1.ipcMain.handle('transform-content', async (_event, args) => {
            try {
    ...
    if (!electron_1.ipcMain.listenerCount('get-keyword-expansions')) {
        electron_1.ipcMain.handle('get-keyword-expansions', async (event, keyword, options) => {
            try {
                ...
                const isValidSearchKeyword = (kw) => {
                    ...
                    if (trimmed.length < 3 || trimmed.length > 40)
                        return false;
                    // 원본 키워드와 동일하면 제외
                    if (trimmed.toLowerCase() === trimmedKeyword.toLowerCase())
                        return false;
                    // ============================================
                    ...
                    if (/^\d+[%조억원회차세대주차월분기]?$/.test(trimmed))
                        return false;
                    // 숫자 비율이 50% 이상이면 제외 (p31, 56% 등)
                    const digitCount = (trimmed.match(/\d/g) || []).length;
                    if (digitCount / trimmed.length > 0.5 && trimmed.length < 8)
                        return false;
                    // ============================================
                    ...
                    if (!trimmed.toLowerCase().includes(seedWord) && trimmed.length < 10) {
                        if (incompleteEndings.some(pattern => pattern.test(trimmed)))
                            return false;
                    }
                    // 불완전한 동사형 어미 (질문형/진행형)
                    const verbFragments = [
                    ...
                    if (verbFragments.some(p => p.test(trimmed)) && trimmed.length < 12)
                        return false;
                    // ============================================
                    // 5️⃣ 뉴스/기사 제목 조각 필터
                    // ============================================
                    ...
                    if (genericWords.includes(trimmed) && !trimmed.includes(' '))
                        return false;
                    // ============================================
                    ...
                    if (containsSeed) {
                        ...
                        const junkTailTokens = new Set([
                            '갤', '룰', '칼', '죽', '팀', '후', '툴', '팩', '짤', '썰', '짤방', '토', '봄', '빵'
                        ]);
                        if (parts.length >= 2) {
                            if (last.length <= 1)
                                return false;
                            if (junkTailTokens.has(last))
                                return false;
                        }
                        if (trimmed.length >= 4)
                            return true;
                ...
                const sendProgress = (step, current, total, message, customPercent) => {
                    let percent = 0;
                ...
                try {
                    if (shouldComputeMetrics) {
                        const baseVolumeData = await (0, naver_datalab_api_1.getNaverKeywordSearchVolumeSeparate)({
                            clientId: naverClientId,
                            clientSecret: naverClientSecret
                ...
                try {
                    ...
                    const uniqueAutocomplete = new Set();
                    // 1. 기본 자동완성
                    const autocompleteKeywords = await (0, naver_autocomplete_1.getNaverAutocompleteKeywords)(trimmedKeyword, {
                        clientId: naverClientId,
                        clientSecret: naverClientSecret
                    });
                    ...
                    if (isUnlimited || targetCount > 200) {
                        ...
                        for (const jamo of jamos) {
                            try {
                                ...
                                const extAuto = await (0, naver_autocomplete_1.getNaverAutocompleteKeywords)(extKeyword, {
                                    clientId: naverClientId,
                                    clientSecret: naverClientSecret
                                });
                                extAuto.forEach(kw => {
                    ...
                    if (uniqueAutocomplete.size < desiredAutocompleteCount) {
                        const seedQueue = [trimmedKeyword, ...Array.from(uniqueAutocomplete).slice(0, 60)];
                        ...
                        for (const seed of seedQueue) {
                            if (uniqueAutocomplete.size >= desiredAutocompleteCount)
                                break;
                            if (calls >= maxSeedCalls)
                                break;
                            const s = String(seed || '').replace(/\s+/g, ' ').trim();
                            const seedKey = s.toLowerCase();
                            if (!s || visitedSeeds.has(seedKey))
                                continue;
                            visitedSeeds.add(seedKey);
                            try {
                                const ext = await (0, naver_autocomplete_1.getNaverAutocompleteKeywords)(s, {
                                    clientId: naverClientId,
                                    clientSecret: naverClientSecret
                                });
                                for (const raw of (ext || [])) {
                                    ...
                                    if (!t)
                                        continue;
                                    if (!isValidSearchKeyword(t))
                                        continue;
                                    if (!uniqueAutocomplete.has(t))
                                        uniqueAutocomplete.add(t);
                                    if (uniqueAutocomplete.size >= desiredAutocompleteCount)
                                        break;
                                }
                            }
                    ...
                    for (let i = 0; i < autocompleteArray.length; i += 5) {
                        if (!isUnlimited && allKeywords.length >= targetCount)
                            break;
                        ...
                        if (shouldComputeMetrics) {
                            let volumeData = null;
                            try {
                                volumeData = await (0, naver_datalab_api_1.getNaverKeywordSearchVolumeSeparate)({
                                    clientId: naverClientId,
                                    clientSecret: naverClientSecret
                            ...
                            for (let j = 0; j < effectiveBatch.length; j++) {
                                ...
                                const totalVol = (pcVol !== null || mobileVol !== null)
                                    ? ((pcVol ?? 0) + (mobileVol ?? 0))
                                    : null;
                                allKeywords.push({
                                    keyword: kw,
                ...
                try {
                    ...
                    const relatedKeywords = await (0, naver_datalab_api_1.getNaverRelatedKeywords)(trimmedKeyword, {
                        clientId: naverClientId,
                        clientSecret: naverClientSecret
                    }, { limit: 50 }); // 더 많이 수집
                    ...
                    if (isUnlimited && validCategoryKeywords.length > 0) {
                        ...
                        for (let i = 0; i < validCategoryKeywords.length; i++) {
                            ...
                            if (allKeywords.length >= targetCount)
                                break;
                            try {
                                // 카테고리 키워드로 자동완성 수집
                                const catAuto = await (0, naver_autocomplete_1.getNaverAutocompleteKeywords)(catKw, {
                                    clientId: naverClientId,
                                    clientSecret: naverClientSecret
                                });
                                for (const kw of catAuto) {
                                    ...
                                    if (trimmed.length >= 3 &&
                                        ...
                                        // 검색량 조회
                                        try {
                                            const volData = await (0, naver_datalab_api_1.getNaverKeywordSearchVolumeSeparate)({
                                                clientId: naverClientId,
                                                clientSecret: naverClientSecret
                                ...
                                // 진행률 업데이트
                                if (i % 5 === 0) {
                                    sendProgress('category-expand', i + 1, validCategoryKeywords.length, `🚀 "${catKw}" 확장 중... (총 ${allKeywords.length}개)`);
                                }
                ...
                if (needsMore) {
                    ...
                    const processInBatches = async (items, processFn) => {
                        ...
                        for (let i = 0; i < items.length; i += batchSize) {
                            if (additionalKeywords.length >= maxAdditional)
                                break;
                            ...
                            // 진행률 업데이트
                            if (i % 20 === 0) {
                                sendProgress('additional', additionalKeywords.length, maxAdditional, `⚡ ${allKeywords.length + additionalKeywords.length}개 수집 중... (${Math.round(i / items.length * 100)}%)`);
                            }
                        }
                        return results;
                    };
                    for (const suffix of suffixes) {
                        // 무제한이 아니면 목표 도달 시 중단
                        if (!isUnlimited && allKeywords.length + additionalKeywords.length >= targetCount)
                            break;
                        // 무제한이어도 최대치 도달 시 중단
                        ...
                        try {
                            ...
                            const extAutoComplete = await (0, naver_autocomplete_1.getNaverAutocompleteKeywords)(extendedKeyword, {
                                clientId: naverClientId,
                                clientSecret: naverClientSecret
                            });
                    ...
                    if (isUnlimited && additionalKeywords.length < maxAdditional) {
                        ...
                        for (const baseKw of topKeywordsFor2nd) {
                            ...
                            try {
                                // 2차 키워드로 자동완성 수집
                                const secondAuto = await (0, naver_autocomplete_1.getNaverAutocompleteKeywords)(baseKw, {
                                    clientId: naverClientId,
                                    clientSecret: naverClientSecret
                                });
                    ...
                    if (isUnlimited && additionalKeywords.length < maxAdditional) {
                        ...
                        for (const suffix of extraSuffixes) {
                            ...
                            try {
                                ...
                                const thirdAuto = await (0, naver_autocomplete_1.getNaverAutocompleteKeywords)(extKeyword, {
                                    clientId: naverClientId,
                                    clientSecret: naverClientSecret
                                });
                    ...
                    if (shouldComputeMetrics) {
                        ...
                        for (let i = 0; i < additionalKeywords.length; i += 5) {
                            ...
                            if (i % 50 === 0) {
                                sendProgress('additional', i, additionalKeywords.length, `📊 검색량 조회 중... (${allKeywords.length}개)`);
                            }
                            await new Promise(resolve => setTimeout(resolve, 50));
                        }
                    }
                ...
                const fetchDocumentCount = async (keyword, maxRetries = 3) => {
                    ...
                    for (let retry = 0; retry < maxRetries; retry++) {
                        try {
                            ...
                            if (verboseDocLog)
                                console.log(`[DOC-COUNT] 📡 API 호출 (${retry + 1}/${maxRetries}): "${keyword}"`);
                            ...
                            if (verboseDocLog)
                                console.log(`[DOC-COUNT] 응답 상태: ${docCountRes.status} ${docCountRes.statusText}`);
                            if (docCountRes.ok) {
                                try {
                                    const docData = (await docCountRes.json());
                                    if (verboseDocLog)
                                        console.log(`[DOC-COUNT] 파싱된 데이터: total=${docData.total}, display=${docData.display}, start=${docData.start}`);
                                    const count = docData.total;
                                    // total이 undefined가 아니고 숫자인 경우에만 반환
                                    if (typeof count === 'number') {
                                        if (verboseDocLog)
                                            console.log(`[DOC-COUNT] ✅ "${keyword}" 문서수: ${count.toLocaleString()}`);
                                        return count;
                                    }
                                    else {
                                ...
                                try {
                                    ...
                                    if (verboseDocLog)
                                        console.warn(`[DOC-COUNT] 에러 내용: ${errorText}`);
                                }
                                catch {
                                    // ignore
                                }
                                ...
                                if (docCountRes.status === 429) {
                                    ...
                                    if (verboseDocLog)
                                        console.log(`[DOC-COUNT] ⏳ Rate Limit! ${backoffMs}ms 대기...`);
                        ...
                        if (verboseDocLog)
                            console.log(`[DOC-COUNT] ⏳ ${waitTime}ms 대기 후 재시도...`);
                        await new Promise(resolve => setTimeout(resolve, waitTime));
                    }
                ...
                if (!shouldComputeMetrics) {
                    ...
                    let nextIndex = 0;
                    let doneCount = 0;
                    const out = new Array(allKeywords.length);
                    const worker = async () => {
                        while (true) {
                            const i = nextIndex++;
                ...
                const isGoldenKeyword = (k) => {
                    ...
                    if (searchVol === null || docCount === null || ratio === null)
                        return false;
                    if (searchVol < 100)
                        return false;
                    if (docCount <= 0)
                        return docCount === 0 && searchVol > 0;
    ...
    // 수식어 키워드 검색 핸들러
    // ========================================
    if (!electron_1.ipcMain.listenerCount('search-suffix-keywords')) {
        electron_1.ipcMain.handle('search-suffix-keywords', async (event, options) => {
            try {
                ...
                const realtimeKeywords = await (0, realtime_search_keywords_1.getNaverRealtimeKeywords)(50);
                ...
                for (const seedKeyword of seedKeywords) {
                    ...
                    try {
                        ...
                        if (volumeData && volumeData.length > 0 && volumeData[0]) {
                            ...
                            // 황금비율 계산
                            const goldenRatio = (typeof documentCount === 'number' && documentCount > 0 && typeof totalVol === 'number')
                ...
                keywords.sort((a, b) => {
                    ...
                    if (aRatio !== null && bRatio === null)
                        return -1;
                    if (aRatio !== null && bRatio !== null && bRatio !== aRatio)
                        return bRatio - aRatio;
                    return 0;
                });
    ...
    // 블로그 지수 & 작성일자 조회 핸들러
    // ========================================
    if (!electron_1.ipcMain.listenerCount('crawl-blog-index')) {
        electron_1.ipcMain.handle('crawl-blog-index', async (_event, keyword) => {
            try {
                // 라이선스 체크
                const license = await licenseManager.loadLicense();
                if (!license || !license.isValid) {
                ...
                const { crawlBlogIndex } = await Promise.resolve().then(() => __importStar(require('../utils/blog-index-crawler')));
    ...
    // 여러 키워드 일괄 조회
    if (!electron_1.ipcMain.listenerCount('crawl-multiple-blog-index')) {
        electron_1.ipcMain.handle('crawl-multiple-blog-index', async (event, keywords) => {
            try {
                console.log(`[BLOG-INDEX] 일괄 조회 시작: ${keywords.length}개 키워드`);
                const { crawlMultipleBlogIndex } = await Promise.resolve().then(() => __importStar(require('../utils/blog-index-crawler')));
    ...
    // 기존 핸들러 제거
    try {
        if (electron_1.ipcMain.listenerCount('analyze-keyword-competition') > 0) {
            console.log('[KEYWORD-MASTER] 기존 analyze-keyword-competition 핸들러 제거 중...');
            electron_1.ipcMain.removeHandler('analyze-keyword-competition');
        }
    }
    catch (e) {
        // 무시
    }
    electron_1.ipcMain.handle('analyze-keyword-competition', async (_event, keyword) => {
        try {
            // 라이선스 체크
            const license = await licenseManager.loadLicense();
            if (!license || !license.isValid) {
            ...
            const { analyzeKeywordCompetition } = await Promise.resolve().then(() => __importStar(require('../utils/keyword-competition/competition-analyzer')));
    ...
    // 연상 키워드 마인드맵 생성 핸들러
    // ========================================
    if (!electron_1.ipcMain.listenerCount('generate-keyword-mindmap')) {
        electron_1.ipcMain.handle('generate-keyword-mindmap', async (event, keyword, options = {}) => {
            try {
                // 라이선스 체크
                const license = await licenseManager.loadLicense();
                if (!license || !license.isValid) {
                ...
                const envManager = environment_manager_1.EnvironmentManager.getInstance();
                const env = envManager.getConfig();
                const { generateKeywordMindmap, extractAllKeywords } = await Promise.resolve().then(() => __importStar(require('../utils/keyword-mindmap')));
                // 🔥 무제한 확장 옵션 처리
    ...
    // 네트워크 최적화 핸들러
    // ========================================
    if (!electron_1.ipcMain.listenerCount('save-network-optimization')) {
        electron_1.ipcMain.handle('save-network-optimization', async (event, settings) => {
            try {
    ...
    // 사용법 영상 관리 핸들러
    // ========================================
    if (!electron_1.ipcMain.listenerCount('get-tutorial-videos')) {
        electron_1.ipcMain.handle('get-tutorial-videos', async () => {
            try {
                const fs = require('fs');
                const path = require('path');
                // 사용법 영상 파일 경로
    ...
    if (!electron_1.ipcMain.listenerCount('save-tutorial-video')) {
        electron_1.ipcMain.handle('save-tutorial-video', async (event, data) => {
            try {
                const fs = require('fs');
                const path = require('path');
                // 사용법 영상 파일 경로
    ...
    // 라이선스 관리 핸들러
    // ========================================
    if (!electron_1.ipcMain.listenerCount('get-license-info')) {
        electron_1.ipcMain.handle('get-license-info', async (_event, options) => {
            try {
    ...
    // 라이선스 서버 동기화 핸들러
    if (!electron_1.ipcMain.listenerCount('refresh-license')) {
        electron_1.ipcMain.handle('refresh-license', async () => {
            try {
    ...
    if (!electron_1.ipcMain.listenerCount('register-license')) {
        electron_1.ipcMain.handle('register-license', async (_event, data) => {
            try {
                ...
                const result = await licenseManager.verifyLicense(data.licenseCode || '', deviceId, serverUrl, data.userId, data.userPassword);
    ...
    if (!electron_1.ipcMain.listenerCount('check-premium-access')) {
        electron_1.ipcMain.handle('check-premium-access', async () => {
            try {
                const license = await licenseManager.loadLicense();
                if (!license || !license.isValid) {
                    return {
    ...
    // ===================================
    // 카테고리별 황금 롱테일 키워드 발굴
    if (!electron_1.ipcMain.listenerCount('hunt-category-longtail-keywords')) {
        electron_1.ipcMain.handle('hunt-category-longtail-keywords', async (_event, options) => {
            try {
                ...
                const envManager = environment_manager_1.EnvironmentManager.getInstance();
                const env = envManager.config || {};
                (0, category_longtail_keyword_hunter_1.setApiConfigs)({
                ...
                let keywords = await (0, category_longtail_keyword_hunter_1.generateCategoryLongtailKeywords)(longtailOptions);
    ...
    // 🔥 백업 롱테일 키워드 함수 (대폭 강화)
    function getBackupLongtailKeywords(category, target) {
        ...
        return keywords.map((kw, index) => ({
            ...
            grade: (index < 2 ? 'SSS' : (index < 5 ? 'SS' : (index < 8 ? 'S' : 'A'))),
            category,
            target,
    ...
    // 사용 가능한 카테고리 목록 조회
    if (!electron_1.ipcMain.listenerCount('get-available-categories')) {
        electron_1.ipcMain.handle('get-available-categories', async () => {
            try {
                const categories = (0, category_longtail_keyword_hunter_1.getAvailableCategories)();
                return {
                    success: true,
    ...
    // 사용 가능한 타겟층 목록 조회
    if (!electron_1.ipcMain.listenerCount('get-available-targets')) {
        electron_1.ipcMain.handle('get-available-targets', async () => {
            try {
                const targets = (0, category_longtail_keyword_hunter_1.getAvailableTargets)();
                return {
                    success: true,
                    targets: targets,
    ...
    // 추천 카테고리-타겟 조합 조회
    if (!electron_1.ipcMain.listenerCount('get-recommended-combinations')) {
        electron_1.ipcMain.handle('get-recommended-combinations', async () => {
            try {
                const combinations = (0, category_longtail_keyword_hunter_1.getRecommendedCombinations)();
                return {
                    success: true,
    ...
    if (!electron_1.ipcMain.listenerCount('get-rising-keywords')) {
        electron_1.ipcMain.handle('get-rising-keywords', async (_event, options = {}) => {
            try {
                console.log('[RISING-KEYWORDS] 급상승 키워드 검색 시작:', options);
                const risingKeywords = await (0, rising_keyword_finder_1.findRisingKeywords)(options.seedKeywords, {
    ...
    if (!electron_1.ipcMain.listenerCount('get-autocomplete-suggestions')) {
        // 🔥 fetch with retry 헬퍼 (100% 성공률 목표!)
        const fetchWithRetryAC = async (url, options, maxRetries = 5) => {
            for (let retry = 0; retry <= maxRetries; retry++) {
                try {
        ...
        electron_1.ipcMain.handle('get-autocomplete-suggestions', async (_event, keyword) => {
            try {
                console.log(`[AUTOCOMPLETE] 🔥 자동완성 조회 (100% 성공률 목표): ${keyword}`);
                const suggestions = [];
                const suggestionSet = new Set(); // 중복 방지
                // 기본 자동완성 - 재시도 포함!
                try {
    ...
    // 🔥 진짜 실시간 급상승 키워드 감지
    if (!electron_1.ipcMain.listenerCount('get-realtime-rising')) {
        electron_1.ipcMain.handle('get-realtime-rising', async () => {
            try {
                console.log('[REALTIME-RISING] 실시간 급상승 감지 시작...');
                const risingKeywords = await (0, realtime_rising_detector_1.detectRealtimeRising)();
                return {
                    success: true,
                    keywords: risingKeywords,
                    count: risingKeywords.length,
    ...
    if (!electron_1.ipcMain.listenerCount('hunt-pro-traffic-keywords')) {
        electron_1.ipcMain.handle('hunt-pro-traffic-keywords', async (_event, options) => {
            try {
                // 🔒 PRO 기능은 1년/영구제만 사용 가능
                const license = await licenseManager.loadLicense();
                // 라이선스 유형 체크 헬퍼 함수
                const checkLicenseType = (type) => {
                    if (!type)
                ...
                const canUsePro = license && license.isValid && (planCheck.isYearOrMore ||
                ...
                // 🔥 PRO 황금 키워드 헌팅 실행
                const result = await (0, pro_traffic_keyword_hunter_1.huntProTrafficKeywords)({
                    ...
                    useDeepMining: options.useDeepMining !== false, // 🔥 딥 마이닝 기본 활성화
    ...
    function getBackupGoldenKeywords(category) {
        const now = new Date();
        const month = now.getMonth() + 1;
        const timestamp = now.toISOString();
        // 🔥 수익 직결 황금 키워드 DB (실제 트래픽 폭발 키워드!)
        const goldenKeywordDB = {
            'all': [
                // 💰 정부 지원금 (항상 검색량 폭발)
        ...
        return keywords.map((kw, index) => ({
            ...
            grade: index < 2 ? 'SSS' : (index < 5 ? 'SS' : 'S'),
            proStrategy: {
    ...
    function generateLiteBackupKeywords(seedKeyword) {
    ...
    function generateSurgingTrendKeywords(seedKeyword) {
        const now = new Date();
        const month = now.getMonth() + 1;
    ...
    // PRO 트래픽 카테고리 목록
    if (!electron_1.ipcMain.listenerCount('get-pro-traffic-categories')) {
        electron_1.ipcMain.handle('get-pro-traffic-categories', async () => {
            return {
                success: true,
                categories: (0, pro_traffic_keyword_hunter_1.getProTrafficCategories)()
            };
        });
        console.log('[KEYWORD-MASTER] ✅ get-pro-traffic-categories 핸들러 등록 완료');
    }
    // 💰 고수익 RPM 키워드 분석 핸들러 (정확한 CPC 데이터 기반)
    if (!electron_1.ipcMain.listenerCount('analyze-keyword-rpm')) {
        electron_1.ipcMain.handle('analyze-keyword-rpm', async (_event, keyword) => {
            try {
                ...
                if (matchedCategory === '일반') {
                    ...
                    const universalTips = [];
                    if (hasBuyIntent)
                        universalTips.push('💰 구매의도 키워드! 비교표와 가격 정보를 상세히 제공하세요.');
                    if (hasInfoIntent)
                        universalTips.push('📖 정보성 키워드! 단계별 가이드와 꿀팁을 제공하세요.');
                    if (hasPolicyIntent)
                        universalTips.push('📋 지원금 키워드! 신청 자격과 방법을 상세히 안내하세요.');
                    if (hasHighValue)
                        universalTips.push('💎 고가 서비스 키워드! 상세 비교와 실제 경험담이 효과적입니다.');
                    if (wordCount >= 3)
                        universalTips.push('🎯 롱테일 키워드! 구체적인 니즈에 맞는 상세한 정보를 제공하세요.');
                    if (universalTips.length === 0)
                        universalTips.push('💡 일반 키워드입니다. 롱테일 확장으로 경쟁력을 높이세요.');
    ...
    if (!electron_1.ipcMain.listenerCount('discover-high-rpm-keywords')) {
        electron_1.ipcMain.handle('discover-high-rpm-keywords', async (_event, category) => {
            try {
                ...
                for (const seed of data.seeds) {
                    try {
                        ...
                        const searchVolumeForCalc = searchVolume ?? 0;
                        let scoreBonus = 0;
                        if (searchVolumeForCalc > 50000)
                            scoreBonus = 5;
                        else if (searchVolumeForCalc > 20000)
                            scoreBonus = 3;
                        else if (searchVolumeForCalc > 5000)
                            scoreBonus = 1;
                        else if (searchVolumeForCalc < 1000 && searchVolumeForCalc > 0)
                            scoreBonus = -3;
    ...
    // 📰 네이버 실시간 인기 뉴스 핸들러
    if (!electron_1.ipcMain.listenerCount('get-naver-popular-news')) {
        electron_1.ipcMain.handle('get-naver-popular-news', async () => {
            try {
                console.log('[NAVER-NEWS] 네이버 실시간 인기 뉴스 조회 시작...');
                const result = await (0, naver_news_crawler_1.getNaverPopularNews)();
                if (result.success) {
    ...
    // 🔍 연상 키워드 → 황금키워드 발굴
    // ===================================
    if (!electron_1.ipcMain.listenerCount('hunt-golden-from-related')) {
        electron_1.ipcMain.handle('hunt-golden-from-related', async (_event, keyword) => {
            try {
                    ...
                    try {
                        ...
                        if (response.ok) {
                            ...
                            items.forEach((item) => {
                                const title = (item.title || '').replace(/<[^>]*>/g, '').trim();
                                // 2-5단어 조합 추출
                                const words = title.split(/[\s,\-\[\]()｜|/·]+/).filter((w) => w && w.length >= 2);
                ...
                for (const kw of relatedKeywords) {
                    ...
                    if (isGolden || goldenKeywords.length < 10) {
                        ...
                        let description = '';
                        if (goldenRatio >= 10)
                            description = '검색량 대비 경쟁이 매우 낮아 진입하기 좋은 키워드입니다!';
                        else if (goldenRatio >= 5)
                            description = '검색량은 적당하고 경쟁이 낮은 황금 키워드입니다.';
                        else if (goldenRatio >= 2)
                            description = '경쟁이 낮아 상위 노출 가능성이 높습니다.';
                        else
                            description = '잠재력이 있는 키워드입니다.';
                        goldenKeywords.push({
                            keyword: kw,
                            searchVolume,
                            documentCount,
    ...
    // ========================================
    if (!electron_1.ipcMain.listenerCount('search-kin-questions')) {
        electron_1.ipcMain.handle('search-kin-questions', async (_event, params) => {
            try {
                // 파라미터 파싱 (레거시 지원)
                let tabType = 'popular';
                ...
                // 라이선스 체크 (3개월 이상)
                const license = await licenseManager.loadLicense();
                const isActuallyPremium = license && license.isValid && (license.plan === '3months' ||
                    license.plan === '1year' ||
                    license.plan === 'unlimited' ||
                    license.licenseType === '3months' ||
                    license.licenseType === '1year' ||
                ...
                // 🔥 v6.0 황금 질문 헌터 - 4개 탭!
                const { fullHunt, getPopularQnA, getRisingQuestions, getTrendingHiddenQuestions } = await Promise.resolve().then(() => __importStar(require('../utils/naver-kin-golden-hunter-v3')));
                let result;
                // 탭별 처리 (4개 탭)
                // popular: 많이 본 Q&A (무료)
                // latest: 급상승 질문 (무료)
    ...
    // 🌊 키워드 흐름 분석 (연상 키워드)
    if (!electron_1.ipcMain.listenerCount('analyze-keyword-flow')) {
        electron_1.ipcMain.handle('analyze-keyword-flow', async (_event, keyword) => {
            try {
                console.log(`[KEYWORD-FLOW] 🌊 키워드 흐름 분석: "${keyword}"`);
                const { analyzeKeywordFlow } = await Promise.resolve().then(() => __importStar(require('../utils/keyword-flow-analyzer')));
    ...
    async function generateRelatedKeywordsAsync(keyword) {
        try {
            const related = await (0, related_keyword_cache_1.getRelatedKeywords)(keyword);
    ...
    function generateBackupGoldenKeywords(category) {
    ...
    electron_1.ipcMain.handle('crawl-news-snippets', async (_event, keyword) => {
        console.log(`[KEYWORD-MASTER] 뉴스 스니펫 크롤링 요청: "${keyword}" (Puppeteer via Main)`);
        try {
            // Main 프로세스에서 Puppeteer 실행
            const snippets = await (0, naver_search_crawler_1.crawlNewsSnippets)(keyword);
    ...
    electron_1.ipcMain.handle('fetch-real-related-keywords', async (_event, keyword) => {
        console.log(`[KEYWORD-MASTER] 연관 검색어 조회 요청: "${keyword}" (Axios via Main)`);
        try {
            // Main 프로세스에서는 User-Agent 헤더 설정 가능
            const response = await axios_1.default.get('https://ac.search.naver.com/nx/ac', {
                params: {
                    q: keyword,
                    con: 1,
                    frm: 'nv',
    ...
    electron_1.ipcMain.handle('get-niche-keywords', async (_event, options) => {
        console.log('[KEYWORD-MASTER] 틈새 키워드 발굴 요청 수신');
        try {
            const api = (0, fresh_keywords_api_1.getFreshKeywordsAPI)();
    ...
    // 🔄 즉시 수집 실행 (IPC 핸들러)
    electron_1.ipcMain.handle('collect-now', async () => {
    ...
    electron_1.ipcMain.handle('gemini-chat', async (_event, args) => {
    ...
    electron_1.ipcMain.handle('find-ultimate-niche-keywords', async (event, options) => {
        ...
        try {
            ...
            const result = await (0, ultimate_niche_finder_1.findUltimateNicheKeywords)({
                ...options,
