  ...
  try {
    ...
    const payload = window.createPayload ? await window.createPayload({
      ...
      overrides: {
        ...
        h2ImageMode: schedule.h2ImageMode || storedPayload.h2ImageMode || schedule.h2Images?.mode || storedPayload.h2Images?.mode || 'all',
        h2ImageSections: schedule.h2ImageSections || schedule.h2Images || storedPayload.h2ImageSections || storedPayload.h2Images || [],
        h2Images: {
          source: schedule.h2ImageSource || storedPayload.h2ImageSource || 'nanobanana2',
          sections: schedule.h2ImageSections || storedPayload.h2ImageSections || [],
          mode: schedule.h2ImageMode || storedPayload.h2ImageMode || schedule.h2Images?.mode || storedPayload.h2Images?.mode || 'all',
        },
        skipImages: schedule.skipImages === true || storedPayload.skipImages === true || schedule.h2ImageMode === 'none' || storedPayload.h2ImageMode === 'none',
...
window.runImageFxLogin = async function () {
  ...
  try {
    ...
    if (result?.loggedIn) {
      ...
      alert('Google 로그인 실패: ' + (result?.message || '시간 초과'));
    }
  } catch (e) {
...
function setLicenseAccessState(partial = {}) {
...
window.setLicenseAccessState = setLicenseAccessState;
    ...
    if (window.blogger && window.blogger.readLicenseFile) {
      ...
      if (result.ok && result.data) {
        ...
        setLicenseAccessState({
          isFreeTrial: false,
          valid: isValid,
          licenseType: licenseData.licenseType || licenseData.type || null,
  ...
  try {
    ...
    try {
      ...
      setLicenseAccessState({
        ...
        licenseType: status?.type || null,
        expiresAt: status?.expiresAt || null,
        source: 'license-status-refresh-fallback',
      });
    }
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  if (window.blogger && window.blogger.onLog) {
    window.blogger.onLog((line) => {
      ...
      try {
        // v3.8.109: BODY-TRACE-MAIN / AGENT-IMG / WP-PUBLISH / PUBLISH-SIGNAL / FORCED-UPDATE-CHECK 등 진단 로그를 콘솔에 직접 출력
        if (/\[(SPIDER-STEP|PUBLISH-SPIDER|INTERNAL-CONSISTENCY-SPIDER|DISPATCH|DISPATCH-THUMB|PUBLISH-SPIDER-WEB|INTERNAL-CONSISTENCY|GEO-AEO-AUDIT|GEO-AEO-AUDIT-POST|RUN-POST|BODY-TRACE|BODY-TRACE-MAIN|AGENT-IMG|WP-PUBLISH|PUBLISH-SIGNAL|FORCED-UPDATE-CHECK|VERSION|APP|PUBLISH|IMG-HOST)\]/.test(line)) {
          console.log(line);
        }
      } catch {}
