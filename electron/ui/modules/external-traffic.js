...
const PLATFORMS = [
  {
    ...
    promptUser: (src) => `원본 블로그: "${src.title}"\n원본 URL: ${src.url}\n원본 요약: ${_extractExtTrafficSourceText(src) || '(요약 없음)'}\n\n인스타그램 A/B/C 3안을 생성하세요. 글 유형을 자동 분류하고, 첫 줄 후보 10개를 점수화한 뒤 최고 점수 첫 줄을 선택하세요. 해시태그는 3~5개, 이모지는 최대 3개, CTA는 프로필/링크 확인 문장으로만 작성하세요. 핵심 결론·수치는 캡션에서 다 알려주세요 (저장 가치가 도달을 만듭니다).`,
  },
  {
    id: 'threads',
    label: 'Threads',
  ...
  },
  {
    ...
    color: '#03c75a',
    promptSystem: `당신은 한국 네이버 블로그 외부유입 글 에디터입니다. SNS 홍보문이 아니라 검색형 포스트를 씁니다. 1,200~1,700자(체류 2분 30초 목표), 존댓말 정보글, 검색 의도에 맞는 제목, 소제목 2~3개와 표/리스트, 본문에서 핵심 답을 실제로 해결한 뒤 표·예외 전문만 원문으로 유도, 해시태그 5~8개. 반말, 과장, 클릭 강요, 원문에 없는 조건 생성은 금지합니다.`,
...
function extTrafficShowSubtab(subtab) {
  ...
  // v3.8.500: 카드뉴스는 사이드탭으로 나갔다 (showTab('cardnews'))
