...
async function loadPosts() {
  ...
  try {
    ...
    list.innerHTML = state.posts.map((p, i) => `
      <div class="sl-post" data-idx="${i}" style="padding:9px 13px; border-bottom:1px solid rgba(148,163,184,0.08); cursor:pointer; font-size:12.5px; color:#cbd5e1;">
        ${esc(p.title)}
        <span style="color:#64748b; font-size:11px; margin-left:6px;">${esc(String(p.url || '').length)}자</span>
      </div>`).join('');
...
function renderList() {
  ...
  box.innerHTML = state.links.map((l, i) => `
    <div style="display:grid; grid-template-columns:1fr auto; gap:12px; align-items:center; padding:12px 14px; background:rgba(15,23,42,0.6); border:1px solid rgba(148,163,184,0.15); border-radius:11px;">
...
async function retarget(index) {
  const link = state.links[index];
  if (!link) return;
  const next = prompt(`"${link.prettyUrl}" 가 갈 새 주소를 넣어주세요.\n\n지금: ${decodeURIComponent(link.url || '')}`, link.url || '');
  if (!next || next === link.url) return;
  msg('바꾸는 중…', 'slListMsg');
  try {
    const payload = (await window.__buildPublishedPlatformPayload?.('wordpress')) || {};
