...
// CSS 생성 헬퍼 함수 (중복 제거) - 핵 옵션 적용
// 인라인 스타일 적용 헬퍼 함수 (CSS 완전 실패 시 대비)
function applyInlineStyles(html) {
  if (!html) return html;

  try {
    // 기본 인라인 스타일 적용 (가장 중요한 것들만)
    const inlineStyleMap = {
      // 제목 스타일
      '<h1>': '<h1 style="font-size: 2.5rem !important; font-weight: 800 !important; margin: 2rem 0 1rem 0 !important; line-height: 1.2 !important; color: #1a1a1a !important;">',
      '<h2>': '<h2 style="font-size: 2rem !important; font-weight: 700 !important; margin: 1.5rem 0 1rem 0 !important; line-height: 1.3 !important; color: #1a1a1a !important;">',
      '<h3>': '<h3 style="font-size: 1.5rem !important; font-weight: 600 !important; margin: 1.25rem 0 0.75rem 0 !important; line-height: 1.4 !important; color: #1a1a1a !important;">',

      // 문단 스타일
      '<p>': '<p style="font-size: 1.1rem !important; line-height: 1.6 !important; margin: 1rem 0 !important; color: #333 !important;">',

      // 리스트 스타일
      '<ul>': '<ul style="margin: 1rem 0 !important; padding-left: 2rem !important;">',
      '<ol>': '<ol style="margin: 1rem 0 !important; padding-left: 2rem !important;">',
      '<li>': '<li style="margin: 0.5rem 0 !important; line-height: 1.6 !important;">',

      // 테이블 스타일
      '<table>': '<table style="width: 100% !important; border-collapse: collapse !important; margin: 1rem 0 !important;">',
      '<th>': '<th style="padding: 12px !important; background: #f5f5f5 !important; border: 1px solid #ddd !important; font-weight: 600 !important;">',
      '<td>': '<td style="padding: 12px !important; border: 1px solid #ddd !important;">',
    };

    let styledHtml = html;
    for (const [tag, styledTag] of Object.entries(inlineStyleMap)) {
      styledHtml = styledHtml.replace(new RegExp(tag.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi'), styledTag);
    }

    console.log('[PUBLISH] 🛡️ 인라인 스타일 적용됨 (CSS 실패 시 대비)');
    return styledHtml;
  } catch (error) {
...
function generateBloggerLayoutCSS() {
  return `
    /* ========================================
       프리미엄 블로그 디자인 시스템 - 독보적 차별화
       BLOGER-GPT EXCLUSIVE SKIN v2.0
       ======================================== */

    /* 독보적 브랜딩 요소 */
    .blogger-gpt-premium::before {
      content: '✨ AI 퍼스트 블로그';
      position: fixed;
      top: 20px;
      right: 20px;
  ...
  try {
    ...
    try {
      ...
      if (process.platform === 'win32') {
        userDataPath = path.join(process.env.APPDATA || process.env.USERPROFILE || '', 'blogger-gpt-cli');
...
async function uploadExternalImage(bloggerClient, blogId, imageUrl, onLog) {
  try {
    ...
    // media.insert 메서드 존재 확인
    if (!bloggerClient.media || typeof bloggerClient.media.insert !== 'function') {
      onLog?.('⚠️ uploadExternalImage: bloggerClient.media.insert 메서드를 사용할 수 없습니다. Blogger API v3에는 media.insert가 없을 수 있습니다.');
      return null;
    }
...
async function publishToBlogger(payload, title, html, thumbnailUrl, onLog, postingStatus = 'publish', scheduleDate = null) {
  ...
  try {
    ...
    // immediate, publish, live 모두 즉시 발행으로 처리
    let cleanPostingStatus = String(postingStatus || 'publish').toLowerCase().trim();
    
    // immediate → publish 변환
    if (cleanPostingStatus === 'immediate') {
      cleanPostingStatus = 'publish';
      console.log(`[PUBLISH] postingStatus "immediate"를 "publish"로 변환`);
    }
...
.max-mode-article h2 {
  font-size: clamp(1.52rem, 1.8vw + 1rem, 1.9rem) !important;
...
.max-mode-article h3 {
  font-size: clamp(1.28rem, 1.2vw + 0.85rem, 1.45rem) !important;
...
.max-mode-article{ --lba-h2-accent:#ffd700; --lba-h2-bg:rgba(255,215,0,.15); }
.supreme-ai-skin{ --lba-h2-accent:#ffd700; --lba-h2-bg:rgba(255,215,0,.15); }
/* ========================================
...
.blogger-hologram-skin li {
  ...
  text-indent: 0 !important; /* 들여쓰기 제거 */
    ...
    cssContent += resetCss;
    console.log(`[PUBLISH] ✅ 리셋 CSS 스타일을 추가했습니다 (총 ${cssContent.length}자)`);
    ...
    // 🔧 외부 이미지 자동 재호스팅: 첫 번째 이미지가 외부 URL이면 업로드 후 대체
    try {
      const imgMatch = finalContent.match(/<img[^>]*src=["']([^"']+)["'][^>]*>/i);
      if (imgMatch) {
        const firstSrc = imgMatch[1];
        const isExternal = /^https?:\/\//i.test(firstSrc) && !/googleusercontent\.com|blogger\.com/i.test(firstSrc);
        if (isExternal) {
          const uploaded = await uploadExternalImage(blogger, blogId, firstSrc, onLog);
          if (uploaded) {
            finalContent = finalContent.replace(firstSrc, uploaded);
            // 썸네일 후보도 업데이트
            if (!processedThumbnailUrl) processedThumbnailUrl = uploaded;
          }
        }
      }
    } catch (e) {
      onLog?.(`⚠️ 외부 이미지 재호스팅 처리 중 오류: ${e?.message || e}`);
    }
    
    // 3. CSS를 항상 앞에 포함 (CSS가 있으면)
    if (cssContent) {
      finalContent = cssContent + '\n' + finalContent;
      console.log(`[PUBLISH] ✅ CSS 스타일을 최종 HTML에 포함했습니다.`);
    } else {
    ...
    // Blogger에서 HTML을 제대로 렌더링하기 위한 안전한 구조
...
`;
    ...
    if (processedThumbnailUrl && isValidThumbnailUrl) {
      ...
      // 더 강화된 썸네일 중복 검출 로직
      const hasThumbnailInHtml = htmlWithoutCss.includes('main-thumbnail') ||
                                htmlWithoutCss.includes('thumbnail-container') ||
                                htmlWithoutCss.includes('thumbnail-image') ||
                                htmlWithoutCss.includes('article-thumbnail') ||
                                htmlWithoutCss.includes('post-thumbnail') ||
                                htmlWithoutCss.includes('featured-image') ||
                                htmlWithoutCss.includes('hero-image') ||
                                htmlWithoutCss.match(/<img[^>]*src=["'][^"']*thumbnail[^"']*["']/i) ||
                                htmlWithoutCss.match(/<img[^>]*alt=["'][^"']*썸네일[^"']*["']/i) ||
                                htmlWithoutCss.match(/<img[^>]*alt=["'][^"']*thumbnail[^"']*["']/i) ||
                                htmlWithoutCss.match(/<div[^>]*class=["'][^"']*thumbnail[^"']*["']/i) ||
                                htmlWithoutCss.match(/<figure[^>]*class=["'][^"']*thumbnail[^"']*["']/i) ||
                                // 추가 검출: 스타일 속성에 썸네일 관련 텍스트가 있는 경우
                                htmlWithoutCss.match(/<img[^>]*style=["'][^"']*썸네일[^"']*["']/i) ||
                                htmlWithoutCss.match(/<img[^>]*title=["'][^"']*썸네일[^"']*["']/i) ||
                                // ID 기반 검출
                                htmlWithoutCss.match(/id=["'][^"']*thumbnail[^"']*["']/i);

      console.log(`[PUBLISH] [썸네일 체크] HTML에 이미 썸네일 있음: ${hasThumbnailInHtml ? '예' : '아니오'}`);

      if (!hasThumbnailInHtml) {
        const thumbnailHtml = `<div style="margin: 0 0 20px 0; text-align: center; border: 2px solid #e5e7eb; border-radius: 12px; padding: 10px; background: #f9fafb;">
  <img src="${processedThumbnailUrl}" alt="${title || '썸네일 이미지'}" style="max-width: 100%; height: auto; border-radius: 8px; display: block; margin: 0 auto; box-shadow: 0 4px 6px rgba(0,0,0,0.1);" />
</div>`;

        finalHtmlContent += thumbnailHtml;
        console.log(`[PUBLISH] ✅ 썸네일 HTML 추가됨 (${thumbnailHtml.length}자)`);
        console.log(`[PUBLISH] ✅ 썸네일 추가 완료`);
        onLog?.(`[PUBLISH] ✅ 썸네일 추가 완료`);
      } else {
        console.log(`[PUBLISH] ⚠️ HTML에 이미 썸네일이 있어 추가 건너뜀`);
      }
    } else {
      console.log(`[PUBLISH] ℹ️ 유효한 썸네일 없음 - 기본 이미지 추가`);

      // 기본 썸네일 이미지 추가 (사용자가 보여준 예시대로: 흰색 카드, 주황색 테두리, 검은색 텍스트)
...
<svg width="1200" height="630" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="600" y="315" font-family="'Noto Sans KR', 'Malgun Gothic', sans-serif"
    ...
    블로그 포스트
       ...
       alt="썸네일 이미지" style="max-width: 100%; height: auto; border-radius: 8px; display: block; margin: 0 auto; box-shadow: 0 4px 6px rgba(0,0,0,0.1);" />
</div>`;

      finalHtmlContent += defaultThumbnailHtml;
      console.log(`[PUBLISH] ✅ 기본 썸네일 추가됨`);
      onLog?.(`[PUBLISH] ✅ 기본 썸네일 이미지 추가`);
    }
...
<!-- Blogger HTML Content End -->`;
    ...
    */
    ...
    // HTML 엔티티 검증 및 수정
    finalHtmlContent = finalHtmlContent
      .replace(/&(?![a-zA-Z0-9#]+;)/g, '&amp;') // 잘못된 & 문자 수정
      .replace(/</g, '&lt;').replace(/>/g, '&gt;') // HTML 태그를 텍스트로 변환 (실수로 삽입된 경우)
      .replace(/&lt;div&gt;/g, '<div>').replace(/&lt;\/div&gt;/g, '</div>') // 정상적인 태그 복원
      .replace(/&lt;p&gt;/g, '<p>').replace(/&lt;\/p&gt;/g, '</p>')
      .replace(/&lt;h[1-6]&gt;/g, (match) => match.replace('&lt;', '<').replace('&gt;', '>'))
      .replace(/&lt;\/h[1-6]&gt;/g, (match) => match.replace('&lt;/', '</').replace('&gt;', '>'));
    ...
    // Blogger API 제한: 약 200KB (204,800 bytes)
    const MAX_CONTENT_SIZE = 200000; // 200KB
    if (contentSizeBytes > MAX_CONTENT_SIZE) {
      const errorMsg = `콘텐츠 크기가 너무 큽니다: ${contentSizeKB}KB (제한: ${Math.round(MAX_CONTENT_SIZE / 1024)}KB). 콘텐츠를 줄여주세요.`;
...
function getBloggerAuthUrl(payload) {
  try {
    ...
    // 최신 OAuth2 엔드포인트 사용 (v2)
    // urn:ietf:wg:oauth:2.0:oob는 "out-of-band" 리다이렉트 URI로, 
    // Google Cloud Console에서 승인된 리다이렉트 URI로 등록되어 있어야 함
