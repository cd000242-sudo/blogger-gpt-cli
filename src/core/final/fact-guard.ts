...
2. 자료에 없는 수치는 **다른 숫자로 바꾸지 말고**, 숫자를 빼고 서술로 바꾸세요.
   예) "월 최대 50만원을 받습니다" → "지원 금액은 공고와 소득 구간에 따라 달라집니다"
   예) "3월 31일까지 접수" → "접수 기간은 해당 회차 공고에서 확인해야 합니다"
3. 문단의 길이·말투·HTML 태그 구조는 그대로 유지하세요.
4. 고칠 필요가 없는 문단은 결과에 넣지 마세요.
5. 새로운 수치를 만들어내지 마세요. 자료에 없으면 숫자를 쓰지 않습니다.
