...
const ADSENSE_SOURCE_KEYWORDS: RegExp[] = [
  ...
  /기획재정부/g,
  /**
   * v3.8.619 — 목록에 없어서 "출처 부족"으로 잘못 잡히던 기관들.
   *
   * 실사고: 공무원 봉급 글이 인사혁신처를 12회, 기획예산처를 9회 인용하는데도
   * 게이트는 "공공기관 출처 1회"라고 했다(한국은행만 셌다).
   * 없는 문제를 지적하면 진짜 지적까지 같이 무시된다.
   * 기획예산처는 기획재정부의 바뀐 이름이라 옛 이름만 두면 새 글이 전부 걸린다.
   */
  /기획예산처/g,
  /인사혁신처/g,
  /공무원보수위원회/g,
