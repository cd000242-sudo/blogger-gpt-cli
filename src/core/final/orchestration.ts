...
export async function generateUltimateMaxModeArticleFinal(
  ...
  html: string; title: string; labels: string[]; thumbnail: string; qualityReport?: any;
  /** v3.8.735 — 자동 발행 결정. MANUAL_REVIEW 면 발행 창구(publishGeneratedContent)가 같은 본문의 자동 발행을 막는다 */
  ...
  // v3.8.380(R5): startTime 선언과 try 진입은 락 획득 직후로 이동됨 (위 참조)
    ...
    /**
     * 📚 v3.8.735 — 근거 1단계를 **제목보다 먼저** 한다.
     *
     * 실측(2026-09-22, 경주 APEC 글): 근거에 없는 "11월 2일"이 제목에 들어갔다. 제목이 근거보다 먼저 만들어졌기 때문이다.
     * 순서를 바꾼다: 검색 → CLEAN → 근거 판정 → 품질 관문 → Research Packet → **제목** → 제목 사실 관문 → 소제목 → 본문.
     * 제목이 정해진 뒤에만 할 수 있는 것(제목 약속 근거·유료 검색 요약)은 2단계에서 합쳐 장부·패킷을 갱신한다.
     */
    ...
    let factEvidence: FactEvidence = { context: '', provider: 'none', trustLevel: 'none', topic: keyword };
    let naverGrounding = '';
    ...
    const judgedPosts = new Set<any>();
    /** 크롤링 글을 관련도 문에 세운다 — 아직 안 본 것만(2단계에서 유료 요약이 더해질 수 있다) */
    const judgeCrawledPosts = (): void => {
      for (const post of crawledPosts as any[]) {
        if (judgedPosts.has(post)) continue;
        judgedPosts.add(post);
    ...
    if (gate.status === 'GROUNDING_WEAK' && !sourceScope && contentMode !== 'shopping') {
      onLog?.(`[PROGRESS] 22% - 🚦 근거 점검: ${gateMod.describeGate(gate)} → 검색어를 바꿔 다시 찾습니다`);
      ...
      for (const retryQuery of [...new Set(retryQueries)].slice(0, 2)) {
        try {
          ...
          onLog?.(`[PROGRESS] 22% - 🔁 재검색 "${retryQuery}" → 통과 ${(again.items || []).length}건 · 버림 ${(again.rejected || []).length}건`);
        } catch (retryErr: any) {
     ...
     */
    const packetModelSnap = require('./model-use').snapshotModels();
    let researchPacket: any = await packetMod.buildResearchPacket({
      mainKeyword: keyword,
      title: keyword,
    ...
    if (payload.titleMode === 'custom' && fixedTitle) {
       ...
       */
      const makeTitle = async (directive: string): Promise<string> => {
        let t = await generateH1TitleFinal(
          keyword,
          titles,
          demandTitleHint,
          shoppingProductName,
    ...
    if (contentMode === 'shopping') {
      ...
      /**
       * 💰 v3.8.441 — **가격을 못 구한 경우를 따로 다룬다.**
       *
       * 실측(발행글 leadernam.com/…연탄불고기…): 6번 "가격" 섹션이 통째로
       *   "결제 직전 화면에서 비교하세요 / 쿠폰 적용 후 최종 금액을 보세요"
       * 같은 **메타 조언**으로 채워졌다. 상품 가격은 한 줄도 없다.
       *
       * 원인: price_deal 의 필수 요소가 "가격 비교표(정가 vs 할인가 vs 경쟁사)"인데
       * 토스는 가격을 클라이언트에서 불러와 정적 HTML 에 없다(실측 확인). 채울 수
       * 없으니 모델이 '가격표 읽는 법'으로 지면을 때운 것이다. 구매 판단에 아무
       * 도움이 안 되는, 검색해서 온 사람이 이미 아는 얘기다.
       *
       * 지어내게 하면 안 되고(거짓 가격은 신뢰와 제휴 규정 양쪽에 치명적),
       * 그렇다고 빈 조언으로 지면을 낭비할 수도 없다. **가격 대신 '값어치'를
       * 따지게** 한다 — 구성 대비 몇 번 쓰는지, 어떤 상황에서 값을 하는지.
       */
      const knownPriceKrw = Number(((payload as any).affiliateProducts || [])[0]?.priceKrw ?? 0);
      const hasNoPrice = !(knownPriceKrw > 0);
      ...
      const guides = SHOPPING_CONVERSION_MODE_SECTIONS.map((sec, idx) => {
        ...
        /**
         * ⚖️ v3.8.442 — 단점 섹션에서 **반론(objection)과 결격(dealbreaker)을 나눈다.**
         *
         * 근거(2026-08 리서치, 카피라이팅 반론처리):
         *   "반론은 정보나 안심으로 해소되는 망설임(가격·시간·신뢰·적합성)이고,
         *    결격은 구조적 불일치다."
         * 둘을 섞어 쓰면 해소 가능한 걱정까지 "이 제품 별로네"로 읽혀 구매를 막고,
         * 반대로 진짜 안 맞는 사람에게는 경고가 안 된다. 나눠 쓰면 양쪽 다 잡힌다.
         *
         * 발행 글 실측: 5번 섹션이 "양념이 강할 수 있다 / 냉동실 공간이 필요하다"를
         * 같은 무게로 나열했다. 앞은 덜어 먹으면 되는 반론이고, 뒤는 냉동실이 꽉 찬
         * 사람에게는 진짜 결격이다. 독자는 그 차이를 구분받지 못했다.
         */
        if (sec.id === 'honest_cons' && !hasNoReviews) {
          reqs = [
            '단점을 **두 종류로 나눠서** 쓰세요. 섞으면 독자가 판단을 못 합니다.',
            '① [걱정되지만 해결되는 것] — 정보나 요령으로 풀리는 망설임입니다. '
              + '무엇이 걱정인지 쓰고, **바로 이어서 해소 방법**을 주세요. '
              + '예) "양념이 세다는 말이 있는데, 면 위에 얹지 말고 따로 담으면 국물 맛이 안 죽습니다."',
            '② [이런 분은 사지 마세요] — 요령으로 안 풀리는 구조적 불일치입니다. '
              + '해결책을 붙이지 말고 **분명하게 말리세요.** 그게 신뢰를 만듭니다. '
              + '예) "냉동실에 한 칸도 못 비우는 상황이면 사지 마세요. 4팩은 자리를 꽤 차지합니다."',
            '②는 최소 2가지, 각각 **어떤 사람인지 구체적으로** 묘사하세요(가족 수·사용 빈도·보관 환경 등).',
            '단점은 확인된 근거(후기·스펙·구성)에서만 찾으세요 — 지어내지 마세요.',
            '마지막에 "그럼에도 이 사람에게는 여전히 좋은 선택"이라고 대상을 지목해 균형을 잡으세요.',
          ];
        }

        /**
         * v3.8.441 — 가격을 못 구했으면 '가격표 읽는 법'을 쓰지 못하게 막는다.
         * (위 hasNoPrice 주석 참고. hasNoReviews 분기보다 뒤에 둬서 확실히 덮어쓴다.)
         */
        if (hasNoPrice && sec.id === 'price_deal') {
          role = '같은 걸 사본 사람';
          contentFocus = '구성 대비 실사용 횟수 · 어떤 상황에서 값을 하는지 · 무엇과 비교해야 하는지';
          reqs = [
            '⚠️ 이 상품의 가격을 확인하지 못했습니다. 금액·할인율·"최저가"를 **절대 지어내지 마세요.**',
            '🚫 "결제 직전 화면에서 비교하세요", "쿠폰 적용 후 최종 금액을 확인하세요", "배송비를 함께 보세요" 같은 '
              + '**일반적인 쇼핑 요령을 쓰지 마세요.** 검색해서 온 사람은 그걸 이미 압니다. 지면 낭비입니다.',
            '대신 **값어치**를 따지세요: 확인된 구성(수량·용량·중량)으로 몇 번 쓸 수 있는지, '
              + '한 번 쓸 때 얼마어치인지를 독자가 스스로 계산할 수 있게 기준을 주세요.',
            '"이 돈이면 아깝지 않은 경우"와 "그 돈 주고 사면 후회하는 경우"를 각각 구체적인 상황으로 쓰세요.',
            '금액을 모른다는 사실은 **한 번만** 짧게 인정하고 넘어가세요 — 반복하면 글이 변명처럼 읽힙니다.',
          ];
        }
...
🎯 **판단을 독자에게 떠넘기지 마세요 (v3.8.442 — 전환을 가장 크게 깎는 습관)**
전환 데이터가 분명합니다. **"이런 사람에게는 이것을 권한다, 왜냐하면 ~"** 은 구매로 이어지고,
**"당신의 필요에 따라 다르다"** 는 이어지지 않습니다. 독자는 결정을 대신 내려 달라고 온 겁니다.

🚫 아래 형태는 **한 번도 쓰지 마세요** (실제 발행 글에서 반복 검출된 표현입니다)
  · "본인의 사용 목적에 맞게 선택하세요"
  · "~에 따라 판단이 달라집니다" / "~에 따라 다를 수 있어요"
  · "식사(사용) 목적에 맞춰 나뉘어요"
  · "무엇을 우선할지 정하면 선택이 쉬워져요"
  · "각자의 상황에 맞게 고려해 보세요"

✅ 대신 **사람을 지목하고 이유를 붙이세요.**
  ❌ "냉면이 메인인지 고기가 메인인지에 따라 준비량 판단이 달라집니다."
  ✅ "혼자 저녁 먹는 날이 주 3회 이상이면 이걸 사세요. 250g이 딱 1인분이라 남기지 않습니다.
      반대로 네 식구가 고기만 배불리 먹는 집이면 사지 마세요 — 4팩을 한 끼에 다 씁니다."
각 섹션에서 최소 한 번은 **"이런 사람은 사세요 / 이런 사람은 사지 마세요"** 를 이 정도로
구체적인 사람 묘사와 함께 쓰세요.

🔢 **뭉뚱그리지 말고 숫자로 (구체성이 신뢰를 만듭니다)**
"비교적 선명해요", "꽤 크게 느껴질 수 있습니다", "적지 않은 편이에요" 같은 표현 대신
확인된 수치로 쓰세요 — 용량·수량·중량·인원수·횟수·기간.
  ❌ "여러 끼로 나눠 먹기 좋아요"        ✅ "혼자 먹으면 4끼, 둘이면 2끼입니다"
확인 안 된 수치는 지어내지 마세요. 다만 **확인된 수치로 계산할 수 있는 것**(1팩당 인원,
총 사용 횟수 등)은 계산해서 보여주세요. 그게 독자가 스스로 못 하는 일입니다.

⚡ **도입부는 준비운동 없이 결론부터**
숏폼·검색 양쪽에서 첫 문장에서 이탈이 갈립니다. "혹시 ~해보신 적 있나요", "~하려다가도
부담스러우면" 같은 **상황 설명으로 시작하지 마세요.**
  ❌ "저녁 메뉴로 불고기를 고르려다가도 양념 준비가 부담스러우면 결국 다른 메뉴로 돌아서게 돼요."
  ✅ "250g씩 나뉜 4팩이라, 냉면 먹는 날 한 팩만 꺼내 구우면 끝납니다. 양념도 이미 돼 있고요."
첫 두 문장 안에 **이 상품이 무엇이고 무엇을 해결하는지**가 나와야 합니다.`;
...
🚫 **위에 몇 건을 실었는지는 본문에 쓰지 마세요 (v3.8.441 — 실측으로 확인된 문제)**
"확인된 3건", "3건의 후기에서", "이 3건만으로는" 같은 말은 **한 번도 쓰지 마세요.**
그건 우리가 자료를 몇 건 확보했는지에 대한 내부 사정일 뿐, 독자에게는
"이 사람은 후기를 3개밖에 못 봤구나"로 읽혀 글의 신뢰를 스스로 깎습니다.${affReviewTotal > 0 ? `
독자에게 의미 있는 숫자는 **전체 ${affReviewTotal.toLocaleString('ko-KR')}건**입니다. 후기 규모를 말해야 한다면 이 숫자를 쓰세요.` : ''}
개별 후기를 인용할 때는 건수를 세지 말고 "5점 후기에서는…", "한 구매자는…"처럼 **장면으로** 쓰세요.`;
    ...
    // v3.8.735: rawFactMode / factCheckMode 는 제목 앞(근거 1단계)에서 정한다
    ...
    // v3.8.735: factEvidence · naverGrounding · 근거 후보는 제목 앞(근거 1단계)에서 이미 모았다 — 여기서는 제목 약속 근거를 보탠다
     ...
     */
    /**
     * v3.8.735 — 근거 2단계: 제목이 정해진 뒤 보탠 것(제목 약속 근거·유료 검색 요약)을 합쳐 장부·패킷을 **갱신**한다.
     * 패킷의 문장(LLM 정리)은 그대로 두고 수치·날짜(코드 추출)만 다시 뽑는다 — 호출 0회.
     */
    judgeCrawledPosts();
    refreshEvidence(String(h1 || ''));
    pipelineStatus.mark('GROUNDING', gate.status, gateMod.describeGate(gate));
...
`;
    ...
    /**
     * 🔁 v3.8.735 — 비평·수정 루프. 사람이 GPTs 에서 "초안 → 비평 → 고치기 → 다시 비평 → 이제 됐다" 하는 과정을 여기서 한다.
     *   Critic 1(사실·의도·밀도) → 문제 있는 절만 수정 → 재비평 → Critic 2(읽는 품질) → major 만 수정 → 끝.
        ...
        const loop = await loopMod.runCritiqueLoop({
          ...
          moreResearch: async (queries: string[]) => {
            ...
            for (const q of queries.slice(0, 2)) {
              const query = q.includes(core.split(' ')[0] || '') ? q : `${core} ${q}`;
              try {
                const again = await fetchGrounding(query, naverSearch as any, { mainKeyword: keyword, display: 8 });
                evidenceCandidates.push(...(again.items || [])); evidenceRejected.push(...(again.rejected || []));
                added += (again.items || []).length;
                onLog?.(`[PROGRESS] 77% - 🔁 보강 검색 "${query}" → 통과 ${(again.items || []).length}건`);
              } catch (e: any) { console.warn('[CRITIQUE] 보강 검색 스킵:', String(e?.message || e).slice(0, 80)); }
            }
            if (added === 0) return null;
            refreshEvidence(String(h1 || ''));
            refreshPacketValues();
            return { packetText: researchPacketText, evidenceText: evidenceRender.text, items: currentEvidenceItems() };
          },
        });
    ...
    // 7. 해시태그
    const hashtagsRaw = await generateHashtagsFinal(keyword, h2Titles);
    /**
     * 🏷️ v3.8.735 — 태그 출처를 가른다. 모델이 키워드·소제목만 보고 만든 태그(semanticKeyword)는 본문에 없으면 버리고,
     * 실제 자동완성(actualSearchKeyword) 중 이 글의 핵심어가 든 것을 더한다. "연관검색어"라는 이름은 실제 검색어에만 쓴다.
     */
    let hashtags = hashtagsRaw;
    let keywordProvenance: any = null;
    try {
      const { buildKeywordProvenance } = require('./keyword-provenance');
      keywordProvenance = buildKeywordProvenance({
        mainKeyword: keyword, title: String(h1 || ''), bodyText: articleTextForAux.replace(/<[^>]+>/g, ' '),
        generatedTags: String(hashtagsRaw || '').split(/[,\n]/), actualSuggestions: demandSignals.searchQueries,
      });
      if (keywordProvenance.hashtag.length >= 3) hashtags = keywordProvenance.hashtag.join(', ');
      onLog?.(`[PROGRESS] 75% - 🏷️ 태그 ${keywordProvenance.hashtag.length}개 (본문에 있는 태그 ${keywordProvenance.articleKeyword.length} · 실제 검색어 ${keywordProvenance.actualSearchKeyword.length} · 본문에 없어 버린 모델 태그 ${keywordProvenance.semanticKeyword.length})`);
    } catch (tagErr: any) { console.warn('[TAGS] 출처 구분 스킵:', String(tagErr?.message || tagErr).slice(0, 80)); }

    /**
     * ⚖️ v3.8.735 — 전체 Final QA. 본문이 좋아도 FAQ·요약표·CTA 에서 오류가 다시 들어올 수 있다.
     *   ① FAQ·요약표의 값을 근거와 대조 — 근거에 없는 값이 든 FAQ 항목·요약표 행은 뺀다(새 값을 만들지 않는다)
     *   ② Final Judge — "이대로 발행해도 되는가" 한 가지만 묻는다
     */
    ...
    if (skipImages) {
      ...
      async function generateSingleSectionImage(i: number): Promise<{ dataUrl: string; source: string }> {
        ...
        try {
          ...
          /**
           * 📸 v3.8.442 — 값이 없을 때의 기본값을 **수집 사진 수로** 정한다.
           *
           * 근거(2026-08 리서치): 구글 2026-03 코어 업데이트가 "제품을 실제로 쓰는
           * 모습의 사진"을 유의미한 랭킹 요소로 올렸고, 네이버는 직접 촬영 이미지에
           * 가산점·스톡 이미지에 감점을 준다. AI 생성컷은 그 반대편 신호다.
           *
           * v3.8.401 에 기본값을 i2i 로 둔 이유는 "쿠팡 API 는 대표 이미지 1장뿐이라
           * product-all 이면 같은 사진이 소제목 수만큼 반복된다"였다. 그 이유는
           * **사진이 여러 장이면 성립하지 않는다.** 실제로 v3.8.440 부터 토스에서
           * 15장을 확보한다. 그러니 장수로 판단한다 — 쿠팡(1장)은 자동으로 i2i 로
           * 남고, 토스·네이버는 실제 사진을 쓴다. UI 가 값을 보내면 그 값이 우선이다.
           */
          // 'auto' 는 "정하지 않았다"는 뜻이다 — UI 가 직접 고른 값만 실어 보낸다.
          const rawStrategy = String((payload as any).shoppingImageStrategy || '').trim();
          const explicitStrategy = rawStrategy === 'auto' ? '' : rawStrategy;
          const collectedPhotoCount = ((payload.productImages as string[] | undefined) || []).length;
          const shoppingStrategy = explicitStrategy
            || (collectedPhotoCount >= 3 ? 'product-all' : 'product-i2i');
          if (!explicitStrategy) {
            onLog?.(`   🖼️ 본문 이미지 전략 자동 선택: ${shoppingStrategy === 'product-all'
              ? `수집한 실제 사진 (${collectedPhotoCount}장 확보 — 검색 신뢰도에 유리)`
              : `AI 생성 (수집 사진 ${collectedPhotoCount}장으로는 부족)`}`);
          }
          /**
...
`;
     ...
     * (v3.8.749: URL 글도 이 경로를 탄다 — URL 모드가 따로 쓰던 썸네일 경로는 없어졌다.)
     */
...
오직 HTML 결과만 출력 (설명·마크다운 금지).`;
    ...
    try {
      ...
      /**
       * v3.8.735 — 100점은 자기평가가 아니라 **Hard Gate 통과**를 뜻한다.
       * 실측: 근거에 없는 "11월 2일"이 제목에 들어간 글이 100점이었다 — auditArticle 은 값을 근거와 대조하지 않는다.
       * 관문이 하나라도 FAIL 이면 점수 상한을 둔다(원점수는 auditScoreRaw 로 남긴다).
       */
      const gatedScore = hardGatesAllPass ? audited.score : Math.min(audited.score, 89);
      appendLedgerEntry(ledgerPath(), {
        ...
        hardGates,
        // v3.8.735 — 비평·수정 루프 기록
        ...(critiqueReport ? {
          draftModel: String(draftModel || ''),
          critic1Model: (critiqueReport.models?.critic1 || []).join(', '),
