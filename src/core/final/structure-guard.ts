...
/**
 * 명사가 아닌 것을 걸러 낸다 — 조사·어미가 붙은 어절.
 *
 * ## 두 번 조정했다 (발행글 142편 전수 실측)
 *
 * **1차 — 너무 넓었다.** 처음엔 조사만 걸렀더니 293건이 나왔는데 표본이 전부 용언이었다:
 *   입양했는지 · 가능하면 · 현실적인 · 파헤치기 · 무엇인가요 · 확인하나요 · 반영될까
 * 이런 경고 293건은 아무도 안 읽는다.
 *
 * **2차 — 너무 좁혔다.** 한 글자 어미(서·고·지·기·요·인)까지 걸렀더니
 * **진짜 명사가 통째로 죽었다**: 진료확인**서** · 증명**서** · 신**고** · 공**고** ·
 * 필**요** · 개**인** · 복**지**. 서류 이름은 이 도메인의 핵심 명사다.
 *
 * **지금 — 명사와 겹치지 않는 용언 어미만.** 두 글자 이상이라 충돌이 없다.
 * 잡아야 할 것들은 다 남는다: 방문판매법 · 대출규제 · 소비자분쟁해결기준 · 피해보상 · 진료확인서.
 */
const HEADING_TAIL = new RegExp(
  '('
  // 조사
  + '을|를|이|가|은|는|의|에|로|으로|과|와|한|된|할|들|하기|보기|기의'
  + '|부터|에서|까지|마다|조차|이나|라도|처럼|보다|에게|한테|으로는|에서는|나|든|든지|랑|에도|로도|서도|에는|에게도'
  // 용언 어미 — 두 글자 이상이라 명사와 겹치지 않는다
  + '|는지|으면|하면|적인|하게|가요|나요|될까|데도|다면|지만|면서|해서|하여|하는|되는|이라|니다|였다|겠다'
  + ')$',
);
