...
const AI_FIXABLE = new Set([
  ...
  // v3.8.662 — 깊이·목소리. 사장님이 허락한 1회 호출을 가장 아픈 곳에 쓴다
  'no-stance',
  'deferral-flood',
  'stance-shallow',
  // v3.8.670 — 사장님 발행글 비평: 약속 절이 "확인하세요" 로만, 같은 확인 절차가 절마다
  'promise-deferred',
  'procedure-repeat',
  'inline-faq',
  'title-thread-lost',
  // v3.8.673 — 실. 26편 보정 뒤 감점과 함께 켠다. 결론 검사는 낱말 대조라 아직 알리기만
...
const FIX_HINTS: Record<string, string> = {
  ...
  'no-stance': '이 구간을 필자의 판단 한 문장으로 닫습니다: 조건(누가·어떤 경우) + 행동(무엇을 먼저 한다) + 이유(자료의 어느 사실 때문인지). "…쪽입니다" 로 끝내지 않습니다.',
  'deferral-flood': '"확인하세요·문의하세요·따라 다릅니다" 로 닫은 문장을 조건별 판단으로 바꿉니다: "A면 X, B면 Y". 근거에 없으면 "확인되지 않았다" 고 밝히되 판단을 비우지 않습니다.',
  'promise-deferred': '이 절은 제목이 약속한 것에 답하는 절입니다. "확인하세요·대조하세요" 로 끝내지 말고 대상별로 답을 적습니다 — "별도 신청 없이 반영되는 사람은 …, 직접 신청해야 하는 사람은 …, 면제인데 고지서가 왔으면 …" 처럼. 자료에 없는 조건은 "자료에서 확인되지 않는다" 한 문장으로 밝히고 공식 확인처를 하나만 적습니다.',
  'procedure-repeat': '같은 확인 절차(예: 고지서와 등록원부 대조)를 이 절에서 다시 쓰지 않습니다. 앞 절에서 한 번 적었으니 여기서는 이 절만의 조건·대상·서류를 씁니다.',
  'stance-shallow': '판단 문장에 조건과 행동을 넣어 다시 씁니다. "경남 사업자라면 10월 안내를 기다리지 말고 지금 손해보험 공고부터 읽으세요. 신용생명보험은 대출이 있을 때만 의미가 있기 때문입니다." 처럼.',
  'inline-faq': '이 구간의 질문·답 목록을 뺍니다. 글 끝의 FAQ 가 따로 있습니다. 그 자리는 이 절에서만 할 수 있는 설명과 판단으로 채웁니다.',
  'intro-question-missing': '서론의 마지막 문장을 독자의 문제를 세우는 질문 한 문장(물음표)으로 바꿉니다. "확인하는 것이 출발점" 같은 지시로 끝내지 않습니다. 답은 절과 결론이 줍니다.',
  'section-closer-checklist': '이 절의 마지막 문단을 필자의 반응으로 바꿉니다 — 조건(누가·어떤 경우) + 행동(무엇을 먼저) + 이유(자료의 어느 사실 때문인지). "점검하세요·확인한 뒤 진행하세요" 목록으로 닫지 않습니다.',
  'conclusion-not-answering': '마무리에서 서론의 질문을 한 문장으로 되받고 답을 줍니다 — "A 라면 된다 / B 라면 안 된다".',
  'title-thread-lost': '이 구간 첫 문장에서 제목이 부른 독자(그 상황의 그 사람)에게 이 절이 무엇을 주는지 잇습니다. 잇지 못하면 제목과 관계없는 내용이니 뺍니다.',
