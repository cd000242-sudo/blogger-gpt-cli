...
export type BlockLanguage = 'ko' | 'en';

/** 알 수 없는 값이 오면 한국어로 둔다 — 기존 동작을 바꾸지 않기 위해서다 */
export function normalizeBlockLanguage(raw: unknown): BlockLanguage {
  const value = String(raw ?? '').trim().toLowerCase();
  if (value === 'en' || value.startsWith('en-') || value.startsWith('en_') || value === 'english') return 'en';
  return 'ko';
}
 ...
 */
export function attachJosa(word: string, withBatchim: string, withoutBatchim: string): string {
...
export interface BlockStrings {
  /** 결론 블록: 질문을 못 받았을 때 키워드로 만드는 문구 */
  answerQuestionFallback(keyword: string): string;
  /** 결론 블록: 근거 줄 앞에 붙는 라벨 */
  answerBasisLabel: string;
  /** 독자 확보 블록 제목 */
  audienceTitle: string;
  /** 독자 확보 블록 설명. siteName 은 **이미 이스케이프된** 문자열이 들어온다 */
  audienceLine(siteName: string): string;
  /** 지정 링크의 글자 */
  audienceCta: string;
  /** 팔로우 안내 */
  audienceFollow: string;
}

const KO: BlockStrings = {
  answerQuestionFallback: (keyword) => `${keyword}, 결론부터`,
  answerBasisLabel: '근거',
  audienceTitle: '이런 글, 놓치지 않으려면',
  audienceLine: (siteName) =>
    `구글에서 <strong>${siteName}</strong>${attachJosa(siteName, '을', '를')} 즐겨 보는 출처로 지정해 두면, ` +
    `다음에 비슷한 내용을 찾을 때 검색 결과 위쪽에서 다시 만날 수 있습니다.`,
  audienceCta: '즐겨 보는 출처로 지정하기',
  audienceFollow:
    '모바일 크롬에서 이 페이지 메뉴의 <strong>팔로우</strong>를 누르면, 새 글이 올라올 때 크롬 첫 화면에 뜹니다.',
};

const EN: BlockStrings = {
  ...
  audienceLine: (siteName) =>
    `Set <strong>${siteName}</strong> as a preferred source on Google. ` +
    `Next time you search for something like this, it shows up higher.`,
  audienceCta: 'Set as preferred source',
  audienceFollow:
    'On mobile Chrome, tap <strong>Follow</strong> in the page menu to get new articles on your home screen.',
};

export function blockStrings(language: BlockLanguage): BlockStrings {
