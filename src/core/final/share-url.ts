 ...
 *
 * ⚠️ v3.8.703 — **속성 순서를 가정하지 않는다.**
 * 예전 규칙은 `data-orbit-share` 가 `href` 보다 **앞에 있어야** 걸렸다.
 * 그런데 티스토리 편집기를 거쳐 돌아온 본문은 속성 순서를 다시 쓴다 —
 * 실측에서 `href` 가 앞선 형태가 나와, 이 저장소의 감사 스크립트조차 공유 버튼을
 * "0개"로 셌다(내가 실제로 사장님께 틀리게 보고했다).
 * 그래서 두 순서를 모두 잡는다.
 */
const SHARE_LINK_URL_PARAM =
  /(<a\b(?=[^>]*\bdata-orbit-share=)[^>]*?\bhref="[^"]*?[?&](?:url|u)=)([^"&]*)/gi;
