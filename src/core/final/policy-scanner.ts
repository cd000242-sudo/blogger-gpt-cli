...
const PROHIBITED_PATTERNS: Array<{ re: RegExp; cat: string }> = [
    ...
    // 도박
    { re: /(불법\s*도박|온라인\s*카지노\s*승률|토토\s*사이트\s*추천|배팅\s*추천|사설\s*토토|먹튀\s*검증)/g, cat: '도박' },
    // 마약
    ...
    // 해킹·악성
    { re: /(해킹\s*도구\s*판매|크랙\s*다운로드|시리얼\s*키\s*공유|키젠\s*다운로드)/g, cat: '해킹' },
    // 저작권 침해 — AdSense 정책 명시 차단 (Programme Policies: Copyright)
    { re: /(토렌트\s*사이트|불법\s*다운로드|영화\s*무료\s*다운|드라마\s*무료\s*다운|만화\s*무료\s*다운|음원\s*불법\s*공유|웹툰\s*무료\s*보기|웹툰\s*불법|불법\s*스트리밍|크랙\s*소프트웨어)/g, cat: '저작권' },
];
...
// ─── 과장·오인 표현 (misleading) ───
//   사용자 실제 표현은 "무조건 100% 수익 보장!", "100% 환급 보장" 등 사이에 다른 단어가 끼어드는 경우 다수.
//   따라서 100%·무조건 패턴은 사이에 0~10자 한글/숫자/% 허용으로 false negative 최소화.
const MISLEADING_PATTERNS: Array<{ re: RegExp; reason: string }> = [
    { re: /100\s*%\s*[가-힣%\d\s]{0,10}?(보장|성공|합격|확실|완벽|환급|수익|이득|당첨)/g, reason: '근거 없는 절대적 보장' },
    { re: /절대\s*(안\s*망함|실패\s*없음|손해\s*없음|손실\s*없음)/g, reason: '근거 없는 절대 부정' },
    { re: /무조건\s*[가-힣%\d\s]{0,10}?(합격|성공|이득|수익|환급|당첨|보장)/g, reason: '무조건 단언' },
    { re: /확정\s*(수익|환급|이득|당첨)/g, reason: '확정 단언' },
