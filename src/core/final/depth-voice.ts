...
3. **자료가 되풀이하는 수치는 문장째 씁니다.** 아래 [자료가 되풀이하는 수치] 에 있는 값은 그 절의 판단 근거로 본문에 그대로 넣습니다.
   ...
   특정 회사의 사내 제도나 다른 상품의 조건이 자료에 섞여 있으면 **그 이름을 아예 쓰지 않습니다** — "사내 제도이므로 적용하면 안 된다" 는
   주의 문장으로도 꺼내지 않습니다 (v3.8.667 실측: 대출 갈아타기 글에 삼성전자 사내 대출이 주의 문장과 FAQ 로 들어왔습니다).
   읽지 않은 주소의 내용을 지어내 "이 기사는 …을 다룹니다" 라고 쓰지 않습니다 — 근거 장부에 본문이 실린 출처만 그 범위 안에서 설명합니다.
...
5. **곁가지 절 금지.** 제목이 부른 독자에게 필요 없는 절(국내 코스닥 보유자 글의 나스닥 절)은 검색어에 딸려 온 것입니다. 그 절은 제목의 독자에게 무엇을 주는지 첫 문장에서 잇지 못하면 뺍니다.
   단, **제목이 부른 것은 곁가지가 아닙니다.** 제목이 "9·4 복합지원센터" 를 약속했으면 그 절은 그날 무슨 일이 있었는지부터 씁니다 — 자료에 있는 그 소식을 "지역 센터 이야기" 라며 밀어내지 마세요.
6. **같은 목록은 한 번만.** 지역·대상·조건 나열("경남, 경북, 광주, 전남, 제주, 충북")은 글 전체에서 한 번만 전부 적고, 뒤에서는 "위 6개 지역" 처럼 가리킵니다.
   ...
   긴 고유명사도 같습니다 — 처음 한 번만 전체로 쓰고 뒤에서는 **줄여 부릅니다** ("고용노동부 2026년 9월 3일 지침" → 그 뒤로는 "지침"). 같은 수치 묶음("200억원→300억원")도 절마다 되풀이하지 않습니다.
7. **연도는 절대 연도로 적습니다.** "다음 해 7월 · 내년 · 올해" 가 아니라 "2027년 7월" 입니다. 독자는 내년에도 이 글을 읽습니다. 자료가 "내년" 이라 해도 글에는 연도로 옮깁니다.
`;
...
// v3.8.670: 해요체 판단("맞다고 봐요·쪽이 맞아요·현실적이에요·권해요")도 판단이다 — 친근한 말투 글에서 판단 0건으로 잘못 셌다
const STANCE_ANY = /제\s*(?:판단|생각|의견|결론)(?:은|으로는?|엔|에는|이|을)|저는\s|제가\s*보기(?:엔|에는)|저라면|(?:맞다고|않다고|맞지\s*않다고)\s*(?:봅니다|봐요)|(?:으로|로)\s*(?:봅니다|봐요)|쪽(?:입니다|이에요)|쪽으로\s*(?:봅니다|봐요)|(?:쪽|편|것|판단|순서|방식|기록)(?:이|은)\s*(?:더\s*)?(?:맞습니다|낫습니다|타당합니다|합리적입니다|자연스럽습니다|현실적입니다|직접적입니다|안전합니다|정확합니다|맞아요|나아요|낫겠어요|타당해요|합리적이에요|자연스러워요|현실적이에요|직접적이에요|안전해요|정확해요|좋아요)|권합니다|권해요|권하지\s*(?:않습니다|않아요)|먼저라는\s*쪽|여기서는\s[^.]{0,80}?(?:맞습니다|낫습니다|타당합니다|봅니다|맞아요|나아요|봐요)/;
// v3.8.664 실측: "…경우에는 …두는 편이 낫습니다" 가 얕다고 세어졌다 — 행동 낱말이 좁았고("두는" 없음), 이유는 안 봤다
export const CONDITION = /(?:라면|이라면|이면|면\s|경우|때는|때에는|일수록|있다면|없다면|받았다면|않았다면|중이라면|전이라면|뒤라면|이상|미만|까지는|부터는|나왔다면|없으면|있으면|거절됐다면|부결됐다면|이때|이런\s*경우|그런\s*경우|그렇다면|그럴\s*때|아니라면|아닌데|이라도|더라도|있는데|없는데)/;
export const ACTION = /(?:신청|접수|내세요|내는|제출|확인|읽|보세요|보는|기다리|미루|먼저|나중|택|고르|바꾸|바꿔|넣|빼|줄이|늘리|묻|문의|정리|대조|분리|나누|피하|말고|하지\s*않|않는|권하|권합|보류|서두르|멈추|시작|짚|검토|비교|두는|두세요|두지|적는|적어|적으|남기|쓰는|쓰지|쓰세요|살피|살펴|찾|맞추|챙기|고치|정정|기록|보관|구분|가르)/;
/** 이유 — 조건이 없어도 "…때문에 …하는 편이 낫습니다" 는 근거 있는 판단이다 (v3.8.664) */
export const REASON = /(?:때문|이유|므로|니까|어서\s|아서\s|해서\s|탓에|덕분에|근거로)/;
