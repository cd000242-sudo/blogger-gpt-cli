...
exports.outputLanguageRule = outputLanguageRule;
exports.systemLanguageLine = systemLanguageLine;
const block_strings_1 = require("./block-strings");
/** 기본은 한국어 — 여태 그렇게 돌았고, 안 주면 동작이 바뀌면 안 된다 */
let activeLanguage = 'ko';
/** 글 하나를 시작할 때 orchestration 이 부른다. 값이 없으면 한국어로 되돌린다. */
function setActiveLanguage(raw) {
    activeLanguage = (0, block_strings_1.normalizeBlockLanguage)(raw);
 ...
 */
function outputLanguageRule(lang = activeLanguage) {
    if (lang === 'en') {
 ...
 */
function systemLanguageLine(lang = activeLanguage) {
    return lang === 'en'
