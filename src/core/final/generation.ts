...
export function sanitizePromptLeaks(html: string): string {
  ...
  const replacements: Array<[RegExp, string]> = [
    /**
     * v3.8.641 — 실제로 새어 나간 문장을 잡는다.
     *
     * 발행글에 이게 그대로 나갔다(2026-09-05 실측, 2회):
     *   "제공된 근거에는 각 상품의 전산심사 기준과 부결 사유가 제시돼 있지 않으므로, …"
     *
     * 아래 두 군데서 빗나갔다:
     *   ① 명사 목록이 `자료|데이터` 뿐이었다 — 실제 문장은 **근거**
     *   ② 문장 끝(`않습니다`)만 봤다 — 실제는 **"않으므로,"** 라는 연결 어미
     *
     * 독자에게 "내가 받은 자료에는 없더라" 를 보고하는 문장이다. 읽는 사람에게
     * 아무 쓸모가 없고, AI 가 쓴 티가 가장 크게 나는 자리다.
     */
    [/(?:제공된|주어진|위)\s*(?:참고\s*)?(?:근거|자료|데이터|정보|출처|문서)(?:\s*장부)?에는?\s*[^.。<]{0,140}?(?:제시(?:되지|돼\s*있지)|나와\s*있지|언급되지|확인되지|포함되지)\s*않(?:아요|았어요|았습니다|습니다|으므로|아서|기\s*때문에|은\s*만큼)[,.]?\s*/g, ''],
    /**
     * v3.8.641 — "…에는 ○○이 **없습니다**" 꼴. 위 규칙은 `않-` 계열만 봤다.
     * 실제 발행글 두 번째 유출이 이 모양이었다:
     *   "제공된 근거에는 부결 뒤 재신청까지 기다려야 하는 기간이나 … 기준이 없습니다."
     */
    [/(?:제공된|주어진|위)\s*(?:참고\s*)?(?:근거|자료|데이터|정보|출처|문서)(?:\s*장부)?에는?\s*[^.。<]{0,140}?없(?:어요|습니다|으므로|어서|기\s*때문에)[,.]?\s*/g, ''],
...
/**
 * 바로 옆에 붙은 **같은 연도**를 한 번으로 줄인다 (v3.8.750 · POST-PUBLISH AUDIT 5865).
 *
 * 실측: 키워드 "2026 신규사업자 카드수수료 환급 조회" → 발행 제목 "2026년 2026 신규사업자 …"(제목·Yoast·slug 모두).
 * 제목 프롬프트의 연도 틀 「"2026년 {키워드} …"」 이 연도를 품은 키워드를 만나 그 꼴을 그대로 지시했다 —
 * 같은 프롬프트로 5번 다시 만들어도 5번 다 그렇게 나왔다. 프롬프트는 두고 여기서 결정적으로 푼다.
 *
 * 줄이는 것: "2026년 2026" · "2026 2026년" · "2026년 2026년" · "2026 2026" (사이에 공백만 있을 때)
 * 두는 것: 다른 연도("2025년 2026년"), 월·회차가 붙은 것("2026년 9월"), "2026년형", 연도가 아닌 숫자("20260명")
 * 「년」이 붙은 쪽을 남긴다 — 제목 연도는 "2026년" 으로 쓴다(제목 프롬프트 규칙).
 */
export function collapseRepeatedYear(title: string): string {
  let out = String(title || '');
  const repeated = /(?<![0-9A-Za-z가-힣])(20\d{2})(년?)\s+\1(년?)(?![0-9A-Za-z가-힣])/g;
  // "2026년 2026년 2026" 처럼 셋 이상이면 한 번에 다 안 줄어든다 — 더 줄지 않을 때까지(최대 3번)
  for (let i = 0; i < 3; i += 1) {
    const next = out.replace(repeated, (_m, year: string, a: string, b: string) => `${year}${a || b}`);
    if (next === out) break;
    out = next;
  }
  return out;
...
export async function generateH1TitleFinal(
  ...
  contentMode?: string,
  /**
   * 📌 v3.8.594 — "키워드를 제목 맨 앞으로" 체크박스.
   *
   * 사용자 지적: "키워드를 제목맨앞으로 체크하고한거야 그렇다면 키워드를 제목맨앞에넣고
   *   자연스러우면서 SEO에 최적화된 제목을 생성해줘야 정상아니니"
   *
   * 맞는 지적이었다. 이 옵션은 여태 **프롬프트에 전달된 적이 없다.** 모델은 옵션을 모른 채
   * 평소대로 제목을 짓고, orchestration 이 나중에 **문자열을 잘라 붙였다**:
   *   키워드에서 제거 → `${keyword} ${나머지}` 로 재조립.
   * 실측 사고 제목 `혁신성장촉진자금 비즈스캔 2026년 신청때 IT 용어 20개인가` 의
   * 그 어색한 이음매가 바로 이 재조립이다. 이제 옵션을 모델에게 알려 주고,
  ...
  /**
   * ✂️ v3.8.594 — 참고 제목은 **꼴만** 보는 것이다.
   *
   * 실측 사고: 키워드가 `혁신성장촉진자금 비즈스캔` 인데 제목이
   * `… 2026년 신청때 IT 용어 20개인가` 로 나왔다. "IT 용어 20개"는 우리 글에서 나온 말이
   * 아니라 참고 제목에서 옮겨 붙은 숫자다. 제목은 본문보다 **먼저** 만들어지므로
   * 그 숫자를 확인할 방법이 없고, 본문은 거꾸로 그 제목을 따라 쓰인다
   * (실제로 본문의 IT 용어는 4개였다).
   */
  const titleReference = crawledTitles.length > 0
    ? `🔍 참고할 인기 제목들 (**꼴만 참고**하세요 — 아래 제목의 숫자·서비스명·상표를 옮겨 적지 마세요):\n${crawledTitles.slice(0, 10).map((t, i) => `${i + 1}. ${t}`).join('\n')}`
...
🔢 **제목에 개수를 약속하지 마세요.** "용어 20개", "방법 7가지" 같은 말은 이 글이 실제로
  그만큼 담을 때만 쓸 수 있는데, 제목은 본문보다 먼저 만들어져 확인할 방법이 없습니다.
  · 참고 제목에 있던 숫자를 옮겨 적지 마세요. 그건 그 글의 숫자입니다.
  · 금액·기간·비율은 근거로 확인된 값만. 확인 안 됐으면 숫자 없이 쓰세요.
...
${keywordFront ? `- 📌 **제목은 반드시 "${keyword}" 로 시작합니다.** 이 글의 필수 조건입니다.
  · 키워드 뒤는 **한 문장으로 자연스럽게 이어져야** 합니다. 조각을 덧붙이지 마세요.
      예) (X) ${keyword} ${currentYear}년 신청때 안내 정리
          (O) ${keyword}, ${currentYear}년에 달라진 게 뭔가요
  · 키워드 다음에 오는 말이 **검색한 사람이 알고 싶어 하는 한 가지**여야 합니다.
  · 연도가 필요한 주제(정책·지원금·세금 등)면 키워드 **바로 뒤**에 "${currentYear}년"을 두세요.
    (연도를 키워드 앞에 두지 마세요 — 이 글은 키워드가 맨 앞입니다)`
    : `- 연도가 필요한 주제(정책·지원금·세금·트렌드 등)면 "${currentYear}년"을 제목 맨 앞에. 불필요한 주제(맛집·일상 꿀팁 등)면 연도 생략.
- 연도를 쓸 때는 반드시 "${currentYear}년" 형태로 제목 맨 앞에. "년 ${currentYear}" 같은 역순 금지.`}
...
9. 🔴 곁가지 금지 (v3.8.662): 제목이 부른 독자에게 필요 없는 섹션 제목은 짓지 마라. 검색어에 딸려 온 다른 시장·다른 지역·다른 제도 이야기는
   제목의 독자가 처한 상황에서 한 걸음씩 답으로 나아가는 제목으로 바꿔라. (단, 제목이 부른 주제는 곁가지가 아니다 — 제목의 낱말이 든 절은 그것부터 다룬다.)
10. 🔴 문답형 섹션 금지 (v3.8.664): "질문과 답변", "Q&A", "자주 묻는 질문", "…을 묻는 이유" 같은 제목을 짓지 마라.
   FAQ 는 코드가 글 끝에 따로 붙인다 — 본문 섹션은 설명과 판단으로 채운다.
...
9. 🔴 곁가지 금지 (v3.8.662): 제목이 부른 독자에게 필요 없는 소제목은 만들지 마세요. 국내 코스닥 보유자 글에 "나스닥 상장폐지 조건" 처럼 검색어에 딸려 온 주제는 뺍니다. 모든 소제목은 제목의 독자가 처한 상황에서 한 걸음씩 답으로 나아가야 합니다. (단, 제목이 부른 주제는 곁가지가 아닙니다 — 제목이 "9·4 복합지원센터" 를 약속했으면 그 소제목은 그 소식을 정면으로 다룹니다.)
10. 🔴 문답형 소제목 금지 (v3.8.664): "질문과 답변", "Q&A", "자주 묻는 질문", "…을 묻는 이유" 처럼 묻고 답하는 소제목은 만들지 마세요. FAQ 는 코드가 글 끝에 따로 붙입니다. 실측: "신용대출 대환 가능성 질문과 답변" 절이 Q/A 다섯 개로 채워졌습니다.
...
- answer: 그 질문의 **답**. 2~4문장, 본문에 있는 숫자·조건을 그대로 쓴다.
  **판정문으로 쓴다** (v3.8.678): "A 라면 됩니다 / B 라면 안 됩니다" 처럼 조건과 결론을 붙인다. 독자가 이 세 줄만 읽고 자기 경우의 답을 알아야 한다.
  "…를 봐요", "…를 확인하세요", "…가 중요해요", "…를 검토할 수 있어요" 같은 볼 것 목록은 답이 아니다 — 금지.
