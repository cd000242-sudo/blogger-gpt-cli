// src/core/url-image-crawler/imageRelevanceScorer.ts
// AI Vision 이미지 관련성 평가 — 다중 vendor (Strategy 패턴)
// 원본: cd000242-sudo/naver — src/crawler/imageRelevanceScorer.ts (v2.7.62)
//
// 핵심:
//   - 글 생성 AI와 동일 vendor로 vision 평가 (Strategy 디스패치)
//   - SSRF 차단 (file:/, private IP)
//   - OOM 차단 (10MB 가드 + 동시성 2)
//   - 실패 시 score=50 중립 반환 (발행은 막지 않음)
//   - 비용 가드 (₩500/₩1500 임계)

import * as https from 'https';
import { VISION_MODELS, routeTextToVision, type VisionRouting } from './visionRouter';
import { chargeAndCheck, getVisionBudget } from './visionBudgetGuard';

// ─── Timeout 인라인 (원본 TimeoutPolicy SSOT 대체) ───
const TIMEOUTS = {
  VL_INFERENCE_FAST: 10_000,     // Gemini Flash, OpenAI mini
  VL_INFERENCE_STANDARD: 12_000, // Claude, OpenAI 41
  VL_INFERENCE_SLOW: 18_000,     // Gemini Pro
};
...
export function parseScoreJson(text: string): { score: number; reason: string } {
  try {
    ...
    return { score: m && m[1] ? Math.min(100, parseInt(m[1], 10)) : 50, reason: 'parse fallback' };
  }
}
...
function geminiStrategy(model: string, apiKey: string): VisionStrategy {
  const timeoutMs = model === VISION_MODELS.GEMINI_PRO ? TIMEOUTS.VL_INFERENCE_SLOW : TIMEOUTS.VL_INFERENCE_FAST;
  return {
    timeoutMs,
...
function claudeStrategy(model: string, apiKey: string): VisionStrategy {
  return {
    timeoutMs: TIMEOUTS.VL_INFERENCE_STANDARD,
    async score(buf, mime, prompt) {
      const body = {
        model,
        max_tokens: 100,
      ...
      }, TIMEOUTS.VL_INFERENCE_STANDARD);
...
function openaiStrategy(model: string, apiKey: string): VisionStrategy {
  const timeoutMs = model === VISION_MODELS.OPENAI_41_MINI ? TIMEOUTS.VL_INFERENCE_FAST : TIMEOUTS.VL_INFERENCE_STANDARD;
  return {
    timeoutMs,
    async score(buf, mime, prompt) {
