...
2. **본문 1단락** (FACT + NATURAL)
   ...
   - "원문 보니까" 같은 link bait 어조 X
   - "~한대" / "~라는데" / "~한다고" 자연 어조

3. **본문 2단락** (HOLD + EMOTION)
   - 작성자 1인칭 반응 (의심/궁금/검증욕구)
   - 비교 자연스럽게 (계산값 OK, 가짜 통계 X)
   - "솔직히 ~인지 의심됨" / "근데 ~ 진짜 가능?"
...
7. 평균 조회수 1만+ 목표라면 그대로 쓰겠는가? YES여야 함
`;
}
...
const VIRAL_CRITIQUE_FIELDS = `      "critique": {
        ...
        "awarenessMatch": 1,
        "notes": "7원칙 통합 평가 한 줄",
        "mustImprove": "발견한 약점 1개 (사실조작/추측/시점조작/클리셰/link bait/원칙 미달)"
      },`;
