...
export const ADSENSE_ULTIMATE_SECTIONS: MaxModeSection[] = [
    ...
    {
      /**
       * ⚠️ v3.8.470 — 이 섹션은 **작성자가 실제로 적어준 경험이 있을 때만** 살아남는다.
       * 경험 없이 "Before & After 구체적 수치" 를 요구하면 겪지 않은 일을 지어내라는
       * 지시가 된다. 그러면 같은 프롬프트의 NO_EXPERIENCE_GUARD("겪은 척 쓰지 마세요")와
       * 정면으로 부딪히고, 어느 쪽이 이길지는 운이 된다.
       * 애드센스 Misleading representation · 공정위 추천보증 심사지침(2026-06-01 시행,
       * AI 가상인물의 허위 경험 서술) 양쪽에 걸리는 지점이라 가드는 필수다.
       */
        ...
        minChars: 1500,
        role: '실전 경험 풍부한 전문가',
        contentFocus: '작성자가 적어준 경험 메모를 읽는 사람에게 쓸모 있게 풀어쓴다',
        requiredElements: [
            '⚠️ 작성자가 적어준 경험 메모 안의 내용만 쓴다 — 없는 수치·날짜·결과를 지어내지 않는다',
            '메모에 있는 상황을 구체적으로 풀어 쓴다 (언제·어디서·무엇이 문제였는지)',
            '그 경험에서 다른 사람이 가져갈 수 있는 판단 기준',
            '메모에 없는 부분은 쓰지 말고 넘어간다 — 분량을 채우려고 만들어내지 않는다',
        ],
    },
