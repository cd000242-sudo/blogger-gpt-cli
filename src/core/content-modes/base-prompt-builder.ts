...
export function getToneInstruction(toneStyle?: string): string {
    if (!toneStyle) return '';
    /**
     * v3.8.674 — 드롭다운 다섯 말투(전문적·격식있는·친근한·대화체·캐주얼)는 tone-registry 가 유일한 정의다.
     * 사장님: "각 이름에 걸맞게 확실한 차이가 있어야지." 여기 있던 TALK(670)·formal 한 줄은 등록부로 옮겼다.
     * expert·review 는 화면에 없는 옛 값 — 아래 한 줄만 남긴다.
     */
    const key = String(toneStyle).toLowerCase().trim();
    if (['professional', 'formal', 'friendly', 'conversational', 'casual'].includes(key)) {
        return require('../final/tone-registry').getToneInstruction(key);
    }
