...
contextBridge.exposeInMainWorld('blogger', {
  ...
  // 🖼️ 이미지 생성 완료 이벤트 (실시간 그리드 업데이트용)
  onImageGenerated: (listener: (data: { slotIndex: number; imageUrl: string; status: 'loading' | 'done' | 'error' }) => void) => {
    const channel = 'image-generated';
    const handler = (_event: Electron.IpcRendererEvent, data: { slotIndex: number; imageUrl: string; status: 'loading' | 'done' | 'error' }) => {
      try {
        listener(data);
      } catch (err) {
        console.error('[preload] onImageGenerated listener error:', err);
      }
    };
    ipcRenderer.on(channel, handler);
    return () => ipcRenderer.removeListener(channel, handler);
