...
const QUERY_SUFFIX: Record<ActionIntent, string> = {
  신청: '온라인 신청 바로가기',
  /**
   * v3.8.578 — 여기 있던 '승차권 예약' 을 뺀다.
   *
   * 실측: "해외 항공권 취소 수수료 면제" 글의 검색어가
   *   "해외 항공권 취소 수수료 면제 예매 바로가기 승차권 예약"
   * 로 만들어졌다. **항공권 글에 승차권을 붙인 것이다.** 그 결과 후보로 코레일 앱,
   * tmoneyGo, 부산시 소식이 올라왔다(관문이 전부 막아 CTA 는 결국 생략됐다).
   *
   * 접미어는 **행동**만 말해야 한다. 무엇을 예매하는지는 키워드가 이미 들고 있고
   * (`base` 에 "항공권"·"코레일"이 들어 있다), 여기서 한 업종의 낱말을 보태면
   * 그 업종으로 검색을 끌고 간다. v3.8.573 의 범용 태그 오배송과 같은 병이다.
   */
  예매: '예매 바로가기',
  예약: '예약 바로가기',
  조회: '조회 바로가기 온라인 확인',
