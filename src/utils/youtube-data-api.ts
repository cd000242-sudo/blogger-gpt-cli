...
export interface YouTubeTrendKeyword {
  keyword: string;
  rank: number;
  viewCount: number;
  changeRate: number;
 ...
 */
export async function getYouTubeTrendKeywords(
  config: YouTubeTrendConfig
): Promise<YouTubeTrendKeyword[]> {
  ...
  try {
          ...
          if (searchResponse.ok) {
            ...
            if (videoIds.length > 0) {
              ...
              if (videosResponse.ok) {
                ...
                const newsVideos = (videosData.items || []).map((item: any) => {
                ...
                // 중복 제거
                const existingIds = new Set(videosWithStats.map((v: any) => v.id));
                const newVideos = newsVideos.filter((v: any) => !existingIds.has(v.id));
      ...
      try {
        ...
        if (trendingResponse.ok) {
          ...
          if (trendingData.items && trendingData.items.length > 0) {
            const trendingVideos = trendingData.items.map((item: any) => {
              const viewCount = parseInt(item.statistics?.viewCount || '0', 10);
              const publishedAt = item.snippet?.publishedAt || '';
              const publishedDate = publishedAt ? new Date(publishedAt) : new Date();
              const hoursSincePublished = Math.max(1, (Date.now() - publishedDate.getTime()) / (1000 * 60 * 60));
            ...
            // 중복 제거
            const existingIds = new Set(videosWithStats.map((v: any) => v.id));
            const newVideos = trendingVideos.filter((v: any) => !existingIds.has(v.id));
            videosWithStats.push(...newVideos);
    ...
    let filteredVideos = videosWithStats.filter((v: any) => v.isNews || v.categoryId === '25');
    ...
    const keywords: YouTubeTrendKeyword[] = finalResults.map((item: any, index: number) => {
 ...
 * 제공된 코드를 참고하여 구현
 */
export async function extractYouTubeTrendingKeywords(
  config: YouTubeTrendConfig
): Promise<string[]> {
  try {
    ...
    trendKeywords.forEach((item) => {
      ...
      const words = cleanTitle
        ...
        .filter((word: string) => {
             ...
             // 개별 단어 추가 (더 많이 포함)
             words.slice(0, 8).forEach((word: string) => {
