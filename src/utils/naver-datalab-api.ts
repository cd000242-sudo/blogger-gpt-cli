...
export async function getNaverKeywordSearchVolumeSeparate(
  config: NaverDatalabConfig,
  keywords: string[]
): Promise<{ keyword: string; pcSearchVolume: number; mobileSearchVolume: number; documentCount: number }[]> {
  if (!config.clientId || !config.clientSecret) {
    throw new Error('네이버 API 인증 정보가 필요합니다');
  }

  const results: { keyword: string; pcSearchVolume: number; mobileSearchVolume: number; documentCount: number }[] = [];
      ...
      const [searchVolumeResult, documentCountResult] = await Promise.allSettled([
        ...
        (async () => {
          ...
          const timeoutPromise = new Promise((_, reject) => 
            setTimeout(() => reject(new Error('검색광고 API 타임아웃 (60초 초과)')), 60000)
          );
