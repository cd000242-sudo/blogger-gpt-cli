...
export async function analyzeKeywordTrendingReason(
  keyword: string,
  keywordData?: { searchVolume?: number; documentCount?: number; growthRate?: number }
  ...
  try {
    ...
    let recentNews: Array<{ title: string; description?: string }> = [];
    
    // 네이버 뉴스 검색 시도 (API 키가 있는 경우)
    if (clientId && clientSecret) {
      try {
        const newsResults = await searchNaverWithApi(
          keyword,
 ...
 */
function extractTrendingReason(
  keyword: string,
  text: string,
    ...
    if (!reason && topTitle.length > 10) {
      ...
      const coreWords = titleWords
        .filter(w => {
          ...
          if (stopWords.some(sw => wLower.includes(sw))) return false;
          // 키워드와 동일한 단어 제외
          if (wLower === keyword.toLowerCase()) return false;
          // 숫자만 있는 경우 제외
 ...
 * 공통 키워드 추출
 */
function extractCommonKeywords(text: string, excludeKeyword: string): string[] {
  const words = text.split(/\s+/).filter(w => {
    const wTrimmed = w.trim();
    if (wTrimmed.length < 2) return false;
    if (wTrimmed === excludeKeyword) return false;
    if (/^\d+$/.test(wTrimmed)) return false;
    return true;
  });
 ...
 * 뉴스 제목에서 직접 이유 추출
 */
function extractReasonFromTitles(news: Array<{ title: string; description?: string }>): string {
  if (news.length === 0 || !news[0]) return '';
 ...
 * 기본 "왜 지금 쓰면 좋은지" 생성
 */
function generateDefaultWhyNow(keywordData: { searchVolume?: number; documentCount?: number; growthRate?: number }): string {
