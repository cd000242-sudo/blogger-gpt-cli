...
class ShoppingCrawler {
   ...
   */
  detectShopType(url) {
    // 🔥 네이버 쇼핑 관련 URL들 우선 처리
    if (url.includes('smartstore.naver.com')) return 'naver-smartstore';
    if (url.includes('brand.naver.com')) return 'naver-smartstore';
    if (url.includes('brandconnect.naver.com')) return 'naver-smartstore';
    if (url.includes('shopping.naver.com')) return 'naver-shopping';
    ...
    if (url.includes('lotteon.com')) return 'lotteon';
    ...
    try {
      switch (shopType) {
        ...
        case 'shoppingconnect':
          // 쇼핑커넥트는 스마트스토어로 리다이렉트되므로 스마트스토어 크롤러 사용
          productInfo = await this.crawlNaverSmartStore(url, options);
          break;
        case '11st':
