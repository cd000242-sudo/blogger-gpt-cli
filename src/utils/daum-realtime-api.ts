  ...
  try {
    ...
    try {
      ...
      await page.waitForSelector(
        '#issueKeyword, #realtimeKeyword, .realtime_keyword, .issue_keyword, .list_issue, .list_news, .news_keyword, [class*="issue"], [id*="issue"], [data-tiara-id*="issue"], [data-tiara-id*="keyword"], a[href*="/search?q="], .rank_issue, .rank_keyword',
    ...
    const keywords = await page.evaluate((maxLimit: number) => {
      ...
      const allKeywords = new Set<string>(); // 중복 방지를 위한 Set
      ...
      // 실시간 이슈 섹션 찾기
      let realtimeSection: Element | null = null;
      ...
      if (realtimeSection) {
        ...
        links.forEach((link, index) => {
          ...
          const isNewsTitle = keyword.length > 30 && (
            keyword.endsWith('했다') || 
      ...
      if (!targetSection || result.length < maxLimit) {
        ...
        const candidates: Array<{ element: Element; keyword: string; rank: number }> = [];
        
        allSearchLinks.forEach((link, index) => {
          if (candidates.length >= maxLimit * 2) return; // 후보 수 제한
      ...
      if (targetSection) {
        ...
        keywordLinks.forEach((el, index) => {
          if (tempKeywords.length >= maxLimit * 3) return;
      ...
      while (result.length < maxLimit && retryCount < maxRetries) {
        ...
        allSearchLinks.forEach((link) => {
          if (additionalKeywords.length >= (maxLimit - result.length) * 3) return;
    ...
    // 결과 변환
    let realtimeKeywords: DaumRealtimeKeyword[] = keywords
      .slice(0, limit)
