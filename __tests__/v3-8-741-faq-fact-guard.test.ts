...
describe('① 질문의 값은 가정값 — 근거를 요구하지 않는다', () => {
  ...
  it('⭐ live 742 재현: 질문은 "7억 주택담보대출", 답변은 "7억원" — 단위만 다른 되받기도 시나리오', () => {
    expect(unsupportedAnswerValues('7억 주택담보대출 월 상환액은 어떻게 계산하나요?', '<p>7억원 주택담보대출의 월 상환액은 적용금리와 상환기간을 넣어 계산기로 확인합니다.</p>', ledger)).toEqual([]);
    const r = guardFaqs([{ question: '7억 주택담보대출 월 상환액은 어떻게 계산하나요?', answer: '<p>7억원 주택담보대출의 월 상환액은 적용금리와 상환기간을 넣어 계산기로 확인합니다.</p>' }], ledger);
    expect(r.faqs).toHaveLength(1);
    // 맨 숫자는 가정값이 되지 않는다 — "3명" 질문이 답변의 "3%" 를 살려 주지 않는다
    expect(unsupportedAnswerValues('동행이 3명인데 예매할 수 있나요?', '<p>수수료는 3%입니다.</p>', ledger)).toEqual(['3%']);
  });
});
