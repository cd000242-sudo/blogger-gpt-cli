...
describe('제목 지시문', () => {
  ...
  /**
   * v3.8.455 에 예시가 바뀌었다.
   *   이전: 좋은 예시 3개가 전부 `${상품명} + 속성 + ~할까` — 모델이 그 표면 형태를
   *         베껴 모든 쇼핑 제목이 한 가지 틀로 수렴했다(실측: 랜선식당·크룹스 글).
   *         게다가 예시가 전부 에어컨 프레임이라 식품·의류 상품에 안 맞았다.
   *   현재: 상황이 먼저 오는 상품군 중립 예시 + "~할까" 어미 금지.
   * 나쁜 예시에는 상품명이 그대로 남는다("이 꼴은 실패"를 보여줘야 하므로).
   */
  it('좋은 꼴과 나쁜 꼴을 보여준다 — 좋은 예시는 상황이 먼저다', () => {
    expect(directive).toContain('이런 꼴이면 실패입니다');
    // 나쁜 예시는 여전히 상품명으로 보여준다
    expect(directive).toContain('브리즈 누비아 이동식 에어컨 핵심 정리');
    // 상품명으로 시작하는 "좋은 예시"는 더 이상 없어야 한다
    expect(directive).not.toContain('브리즈 누비아 이동식 에어컨 전기요금 정말 얼마 나올까');
    expect(directive).toContain('상황');
  });

  it('⭐ 한 가지 틀로 굳지 않게 어미를 막는다 (v3.8.455)', () => {
